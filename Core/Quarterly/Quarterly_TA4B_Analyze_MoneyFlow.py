...
"""
Name: Quarterly_TA4B_Analyze_MoneyFlow.py
Function:
1. 从SQLite数据库读取季线资金流数据
2. 计算生成各种季线资金流技术指标
3. 将计算结果保存到SQLite数据库
4. 生成季线资金流分析图表PDF
5. 使用并行处理进行优化

输入数据表：hk_hist_quarterly_moneyflow 
输出数据表：hk_quarterly_moneyflow_analysis 
生成季线资金流分析图表：{ticker}_quarterly_moneyflow_analysis.pdf
...
def setup_logger(log_dir):
    ...
    """
    log_file = os.path.join(log_dir, 'Quarterly_TA4B_Analyze_MoneyFlow.log')
...
def adjust_date_to_quarter_end(date_value):
    """
    将日期矫正为当季的最后一天
    ...
    Returns:
        str: 格式化为 'YYYY-MM-DD' 的季末日期字符串
    
    Examples:
        >>> adjust_date_to_quarter_end('2026-06-15')
        '2026-06-30'
        >>> adjust_date_to_quarter_end('2026-06-30')
        '2026-06-30'
    """
    # 转换为datetime对象
    ...
    # 获取当季最后一天（下个季度第一天 - 1天）
    quarter_end = pd.Timestamp(dt).to_period('Q').end_time
    
    return quarter_end.strftime('%Y-%m-%d')

def adjust_date_column_to_quarter_end(df, date_column='date'):
    """
    将DataFrame中的日期列矫正为当季的最后一天
    ...
    # 应用日期矫正
    result_df[date_column] = result_df[date_column].apply(adjust_date_to_quarter_end)
    
    # 记录日志
    logger = get_logger()
    logger.debug(f"日期列 '{date_column}' 已矫正为当季最后一天")
    
    return result_df

def get_dynamic_cutoff_date() -> datetime.date:
    """
    根据运行当天日期返回截止日期（自然季末，近似最后一个交易日）：
    - 若当天处于季度末月（3/6/9/12月）且 day >= 25: 返回当季最后一天（视为当季最后一个交易日）
    - 否则: 返回上一季度最后一天（视为上季度最后一个交易日）
    """
    today = datetime.now().date()
    today_ts = pd.Timestamp(today)
    current_quarter = today_ts.to_period('Q')
    if today.month in (3, 6, 9, 12) and today.day >= 25:
        # 当季最后一天
        cutoff_ts = current_quarter.end_time
        period_desc = "当季"
    else:
        # 上季度最后一天
        cutoff_ts = (current_quarter - 1).end_time
        period_desc = "上季度"
    cutoff = cutoff_ts.date()
    logger = get_logger()
    logger.info(f"当前日期 {today}，截止日期设为{period_desc}最后一天（近似）: {cutoff}")
    return cutoff
...
def get_stock_name_from_db(stock_code):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT stock_name FROM hk_hist_quarterly_moneyflow WHERE stock_code = ? LIMIT 1",
            (stock_code,)
        )
...
def load_stock_names_from_json(json_path):
    ...
    try:
        ...
        logger = get_logger()
        logger.info(f"从 {json_path} 加载了 {len(stock_dict)} 个股票名称映射")
        return stock_dict
    except Exception as e:
        logger = get_logger()
        logger.warning(f"加载股票名称映射时出错: {e}")
        return {}
...
def read_moneyflow_from_db(stock_code, start_date=None, end_date=None):
    ...
    try:
        ...
        query = """
            ...
            FROM hk_hist_quarterly_moneyflow
            WHERE stock_code = ?
        """
        params = [stock_code]
...
def save_analysis_to_db(df, stock_code):
    ...
    try:
        ...
        # ===== 关键修改：在保存前进行日期矫正 =====
        # 将日期矫正为当季最后一天
        df_corrected = adjust_date_column_to_quarter_end(df, 'date')
        
        logger = get_logger()
        logger.info(f"{stock_code}: 日期已矫正为当季最后一天")
        ...
        for _, row in df_corrected.iterrows():
            cursor.execute("""
                INSERT OR REPLACE INTO hk_quarterly_moneyflow_analysis 
...
def create_analysis_table_if_not_exists():
    """确保 hk_quarterly_moneyflow_analysis 分析结果表存在"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS hk_quarterly_moneyflow_analysis (
        ...
        logger = get_logger()
        logger.error(f"创建/检查 hk_quarterly_moneyflow_analysis 表失败: {e}")
...
def check_and_process_stock(stock_code):
    ...
    # 获取原始数据最新日期
    source_latest = get_latest_date_from_table('hk_hist_quarterly_moneyflow', stock_code)
    if not source_latest:
        return False, f"{stock_code} 原始数据缺失", None, None
    
    # 获取分析数据最新日期
    analysis_latest = get_latest_date_from_table('hk_quarterly_moneyflow_analysis', stock_code)
...
def calculate_money_flow_indicator_df(df):
    """
    计算季资金流指标（机构/散户资金流、5季度均线、IDR、FBI等）
    注意：在计算前会进行日期矫正
    ...
    Returns:
        pd.DataFrame: 包含计算结果的DataFrame（日期已矫正为季末）
    """
    if df.empty:
        return df
    ...
    # ===== 关键修改：在计算指标前进行日期矫正 =====
    # 将日期矫正为当季最后一天
    result_df = adjust_date_column_to_quarter_end(result_df, 'date')
    ...
    logger = get_logger()
    logger.debug(f"日期已矫正为季末，共处理 {len(result_df)} 条记录")
...
def create_grouped_stacked_bars(current_ticker, date_range_label, period_data_cache, data_periods=None):
    """
    Optimized function to create grouped stacked bar charts with negative value support (Quarterly version)
    """
    ...
    ax.set_title(
        f'{current_ticker} Investor Flow Breakdown Comparison (Quarterly)',
        fontsize=16,
        pad=20
    )
...
def generate_moneyflow_pdf(stock_code, pdf_path, start_date=None, end_date=None):
    """
    生成季线资金流分析图表PDF（使用matplotlib的PDF后端）
    ...
    try:
        ...
        # 确定分析期间（季度版本）
        if end_date is None:
            end_date = get_dynamic_cutoff_date()
        else:
        ...
        available_dates = [d for d in data_dates if d <= end_date]
        ...
        date_range_label = f"{start_date} to {analysis_end_date}"
        
        # 筛选分析期间数据
        mask = (df['Date'].dt.date >= start_date) & (df['Date'].dt.date <= analysis_end_date)
        df_latest = df.loc[mask].copy()
        
        if len(df_latest) < 5:
            logger.warning(f"{stock_code}: 分析期间数据不足5个季度 ({len(df_latest)}个季度)")
...
资金流指标:
- 机构主导季度数: {(inst_flow > ind_flow).sum()}/{len(df_latest)}
- IDR均值: {df_latest['IDR'].mean():.2f}, FBI均值: {df_latest['FBI'].mean():.2f}
- 平均季度涨幅: {df_latest['Price_Chg%'].mean():.2f}%
"""
            ...
            ax1.set_title(f'{stock_code} ({stock_name}) 季资金流趋势\n{date_range_label}')
            ax1.set_xlabel('日期')
            ...
            # ---------- Plot 2: 5-Month Moving Average ----------
            fig2, ax2 = plt.subplots(figsize=(11, 6))
            ax2.plot(df_latest['Date'], df_latest['Inst_5MA']/scale, label='机构5季度均线', color='#1f77b4')
            ax2.plot(df_latest['Date'], df_latest['Ind_5MA']/scale, label='散户5季度均线', color='#ff7f0e')
            ax2.axhline(0, color='black', linestyle='--')
            ax2.set_title(f'{stock_code} ({stock_name}) 5季度移动平均线\n{date_range_label}')
            ax2.set_xlabel('日期')
            ...
            for p in all_periods:
                if isinstance(p, int) and len(df) >= p:
                    period_cache[p] = (f"最新{p}个季度", df.tail(p))
                elif p == 'AP':
...
def process_single_ticker(ticker, CONFIG, start_date=None, end_date=None):
    """
    ...
    try:
        ...
        # 准备PDF路径
        pdf_path = os.path.join(GlobalConfig.full_report_dir, f"{ticker}_quarterly_moneyflow_analysis.pdf")
        os.makedirs(os.path.dirname(pdf_path), exist_ok=True)
        
        # 无论PDF是否存在，都强制重新生成
        if not needs_processing:
            # 数据已存在，强制重新生成PDF（覆盖旧文件）
            if generate_moneyflow_pdf(ticker, pdf_path, start_date, end_date):
        ...
        # 读取原始数据
        df_source = read_moneyflow_from_db(ticker, start_date, end_date)
        if df_source.empty:
        ...
        existing_df = pd.read_sql_query(
            "SELECT * FROM hk_quarterly_moneyflow_analysis WHERE stock_code = ?",
            conn, params=(ticker,)
        )
        conn.close()
...
def process_tickers_parallel(tickers, max_workers=None, use_processes=False, start_date=None, end_date=None):
    """
    并行处理多个股票
    ...
    if use_processes:
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(process_single_ticker, ticker, None, start_date, end_date): ticker 
...
def main():
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("Quarterly_TA4B_Analyze_MoneyFlow 启动")
        logger.info("=" * 80)
        ...
        # ========== 动态计算起止日期 ==========
        # 1. 根据当天日期计算截止日期（季末近似交易日）
        end_date = get_dynamic_cutoff_date()
        
        # 2. 起始日期为截止日期前推20年的当季第1天
        end_ts = pd.Timestamp(end_date)
        start_ts = (end_ts - pd.DateOffset(years=20)).to_period('Q').start_time
        start_date = start_ts.date()
        
        logger.info(f"\n📅 季线资金流分析的日期范围（动态计算）:")
        logger.info(f"   起始日期: {start_date.strftime('%Y-%m-%d')} (20年前当季第1天: {start_ts.strftime('%Y-%m-%d')})")
        logger.info(f"   截止日期: {end_date.strftime('%Y-%m-%d')} (根据当天日期动态确定)")
        logger.info(f"   注意: 如果某只股票数据不足20年，分析时将自动使用其最早可用日期作为起始日期。")
        ...
        logger.info(f"{current_time} 开始分析季资金流指标...")
        ...
        if use_parallel and len(tickers) > 1:
            # 并行处理
            result_stats = process_tickers_parallel(tickers, max_workers, use_processes, start_date, end_date)
            ...
            for ticker in tickers:
                process_single_ticker(ticker, CONFIG, start_date, end_date)

        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        logger.info(f"\n{current_time} ✅ 全部季资金流指标分析完成！")
        logger.info("=" * 80)
