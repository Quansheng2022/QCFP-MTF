...
"""
Name: Quarterly_TA2A_Aggregate_Indicators_akshare.py
Function: 
季线数据聚合处理。
输入数据表：hk_hist_daily_kline
输出数据表：hk_hist_quarterly_kline
"""
...
def setup_logging(log_dir):
    """设置日志配置 - 覆盖模式，时间戳精确到秒"""
    log_file = Path(log_dir) / 'Quarterly_TA2A_Aggregate_Indicators_akshare.log'
...
def get_existing_quarterly_dates(conn, stock_code):
    """获取已存在的季线日期"""
    cursor = conn.cursor()
    try:
        if not check_table_exists(conn, 'hk_hist_quarterly_kline'):
            return set()
        
        cursor.execute("SELECT DISTINCT date FROM hk_hist_quarterly_kline WHERE stock_code = ?", (stock_code,))
        results = cursor.fetchall()
        return {row[0] for row in results if row[0]}
    except sqlite3.OperationalError as e:
        logging.warning(f"查询hk_hist_quarterly_kline表时出错: {e}")
        return set()
...
def get_dynamic_cutoff_date(today, daily_dates, logger):
    ...
    规则（季度版）：
        - 若 today 处于季度末月（3/6/9/12月）且 day >= 25：返回当前季度的最后一个交易日
        - 否则：返回上一季度的最后一个交易日
    ...
    if today.month in (3, 6, 9, 12) and today.day >= 25:
        # 当前季度
        target_period = current_quarter
        period_desc = "当前季度"
    else:
        # 上一季度
        target_period = current_quarter - 1
        period_desc = "上一季度"
    quarter_end = target_period.end_time
    mask = daily_dates <= quarter_end
    last_trading_day = daily_dates[mask].max()
    if pd.isna(last_trading_day):
        raise ValueError(f"{period_desc} ({target_period}) 没有交易日数据，无法确定截止日期")
    logger.info(f"运行日 {today}，截止日期设为{period_desc}的最后一个交易日: {last_trading_day.date()}")
    return last_trading_day


def create_quarterly_table_if_not_exists(conn, logger):
    """创建hk_hist_quarterly_kline表"""
    if check_table_exists(conn, 'hk_hist_quarterly_kline'):
        return True
    
    cursor = conn.cursor()
    create_table_sql = """
    CREATE TABLE IF NOT EXISTS hk_hist_quarterly_kline (
    ...
    conn.commit()
    logger.info("创建 hk_hist_quarterly_kline 表")
    return True


def fix_missing_stock_names(conn, stock_name_map, logger):
    """
    修复数据库中所有空股票名称
    使用 stock_name_map 中的名称填充为空的 stock_name
    """
    cursor = conn.cursor()
    
    total_updated = 0
    for stock_code, stock_name in stock_name_map.items():
        if not stock_name:
            continue
        
        # 更新所有 stock_name 为空或 NULL 的记录
        cursor.execute(
            """
            UPDATE hk_hist_quarterly_kline 
            SET stock_name = ? 
            WHERE stock_code = ? 
        ...
        if updated > 0:
            ...
            logger.info(f"修复股票 {stock_code} ({stock_name}) 的 {updated} 条空名称记录")
    
    if total_updated > 0:
        conn.commit()
        logger.info(f"✅ 共修复了 {total_updated} 条记录的空股票名称")
    else:
        logger.info("✅ 所有记录的股票名称都已完整，无需修复")
    
    return total_updated


def save_quarterly_data_to_db(conn, df, stock_code, stock_name, logger):
    """保存季线数据到数据库"""
    cursor = conn.cursor()
    
    # 确保表存在
    create_quarterly_table_if_not_exists(conn, logger)
    
    # 获取表的列名
    table_columns = get_table_columns(conn, 'hk_hist_quarterly_kline')
    logger.debug(f"表 hk_hist_quarterly_kline 的列: {table_columns}")
    ...
    # 获取已存在的日期
    existing_dates = get_existing_quarterly_dates(conn, stock_code)
    if existing_dates:
        original_count = len(df_to_save)
        df_to_save = df_to_save[~df_to_save['date'].isin(existing_dates)]
        if len(df_to_save) < original_count:
            logger.info(f"跳过 {original_count - len(df_to_save)} 条已存在的季线记录")
        if df_to_save.empty:
            logger.info("没有新的季线数据需要保存")
            return 0
    ...
    sql = f'INSERT OR REPLACE INTO hk_hist_quarterly_kline ({columns_str}) VALUES ({placeholders})'
    ...
    # 验证插入
    cursor.execute("SELECT COUNT(*) FROM hk_hist_quarterly_kline WHERE stock_code = ?", (stock_code,))
...
def aggregate_quarterly_data(daily_df, logger, cutoff_date=None):
    """聚合日线数据为季线数据并计算技术指标"""
    ...
    quarterly_df = daily_df.resample('QE').agg(aggregation)
    quarterly_df.reset_index(inplace=True)
    quarterly_df = quarterly_df.dropna(subset=['open', 'high', 'low', 'close'])
    
    if quarterly_df.empty:
        logger.warning("没有足够的季线数据")
        return None
    
    # 计算技术指标
    quarterly_df['amplitude'] = (quarterly_df['high'] - quarterly_df['low']) / quarterly_df['open'] * 100
    quarterly_df['change_percent'] = quarterly_df['close'].pct_change() * 100
    quarterly_df['change_amount'] = quarterly_df['close'].diff()

    periods = [5, 10, 20, 50, 100, 200]
    for period in periods:
        quarterly_df[f'ema{period}'] = quarterly_df['close'].ewm(span=period, adjust=False).mean()

    ema12 = quarterly_df['close'].ewm(span=12, adjust=False).mean()
    ema26 = quarterly_df['close'].ewm(span=26, adjust=False).mean()
    quarterly_df['macd_dif'] = ema12 - ema26
    quarterly_df['macd_signal'] = quarterly_df['macd_dif'].ewm(span=9, adjust=False).mean()
    quarterly_df['macd_histogram'] = 2 * (quarterly_df['macd_dif'] - quarterly_df['macd_signal'])

    all_columns = quarterly_df.columns.tolist()
    new_columns = ['date'] + [col for col in all_columns if col != 'date']
    quarterly_df = quarterly_df[new_columns]

    logger.info(f"生成 {len(quarterly_df)} 条季线数据 (含技术指标)")
    logger.info(f"季线日期范围: {quarterly_df['date'].min()} 至 {quarterly_df['date'].max()}")
    return quarterly_df
...
def main():
    ...
    try:
        ...
        logger.info(f"日志文件位置: {GlobalConfig.full_log_dir / 'Quarterly_TA2A_Aggregate_Indicators_akshare.log'}")
        ...
        for ticker in tickers_to_process:
            ...
            quarterly_df = aggregate_quarterly_data(daily_df, logger, cutoff_date=cutoff_date)
            if quarterly_df is None or quarterly_df.empty:
                logger.warning(f"股票 {ticker} 季线数据聚合失败，跳过")
                continue
            
            saved_count = save_quarterly_data_to_db(db_connection, quarterly_df, ticker, stock_name, logger)
            if saved_count > 0:
                total_inserted += saved_count
                logger.info(f"成功保存 {saved_count} 条季线记录")
            else:
                logger.info("没有新的季线记录需要保存")
            
            logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} 股票 {ticker} 处理完成")

        # ===== 修复数据库中空的股票名称 =====
        logger.info("\n" + "=" * 70)
        logger.info("🔧 修复数据库中空的股票名称")
        logger.info("=" * 70)
        fix_missing_stock_names(db_connection, stock_name_map, logger)

        # ===== 最终统计 =====
        cursor = db_connection.cursor()
        if check_table_exists(db_connection, 'hk_hist_quarterly_kline'):
            cursor.execute("SELECT COUNT(*) FROM hk_hist_quarterly_kline")
            final_count = cursor.fetchone()[0]
            logger.info(f"\n📊 最终统计: hk_hist_quarterly_kline 表共有 {final_count} 条记录")
            
            # ✅ 修复：按 stock_code 分组，名称从 stock_name_map 获取
            cursor.execute("""
            ...
            for row in cursor.fetchall():
                ...
                logger.info(f"    - {stock_code} ({stock_name}): {row[1]} 条")
        
        db_connection.close()
        logger.info(f"\n{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ✅ 全部完成！共插入 {total_inserted} 条季线记录")
