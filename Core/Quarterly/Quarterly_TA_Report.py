...
# ==================== 路径配置 ====================
# 动态解析项目根目录（本文件位于 Core/Quarterly，向上两级即项目根目录）
PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
SCRIPTS_DIR = os.path.join(PROJECT_ROOT, "Core", "Quarterly")
VENV_DIR = os.path.join(PROJECT_ROOT, ".venv")
LAUNCHER_PATH = os.path.join(PROJECT_ROOT,"Core", "Utl", "stock_analysis_utl.py")

# 日志文件路径配置
LOG_DIR = os.path.join(PROJECT_ROOT, "Log")
LOG_FILE = os.path.join(LOG_DIR, "Quarterly_TA_Report.log")
...
class TeeLogger:
    ...
    def _setup_logging(self):
        ...
        # 设置logger
        self.logger = logging.getLogger('Quarterly_TA_Report')
...
def display_summary(success_count, total_count, failed_scripts, start_time):
    ...
    if failed_scripts:
        ...
        tee_logger.info("\n[SUCCESS] 所有脚本执行成功！")
        tee_logger.info("✅ 季线技术分析报告已生成并合并完成")
...
def main():
    ...
    try:
        ...
        # 要执行的脚本列表（按顺序）- 季线技术分析工作流
        scripts = [
            "Quarterly_TA2A_Aggregate_Indicators_akshare.py",
            "Quarterly_TA2B_Calculate_indicators.py",
            "Quarterly_TA3_Analyze_Indicators_Plot.py",
            "Quarterly_TA4A_Aggregate_MoneyFlow.py",
            "Quarterly_TA4B_Analyze_MoneyFlow.py",
            "Quarterly_TA4C_Analyze_institutional_holdings.py",
            "Quarterly_TA4D_Analyze_chip.py",
            "Quarterly_TA4E_Analyze_chip_report.py",
            "Quarterly_TA5_MergePDF.py"  # 最后一个脚本 - 合并PDF
        ]
        
        tee_logger.info(f"\n{'='*70}")
        tee_logger.info(f"开始顺序执行 {len(scripts)} 个季线分析脚本")
        tee_logger.info(f"开始时间: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
        tee_logger.info(f"工作目录: {os.getcwd()}")
        tee_logger.info(f"Python解释器: {python_exe}")
        tee_logger.info(f"季线分析工作流包括:")
        ...
        for i, script in enumerate(scripts, 1):
            ...
            if run_script(script, python_exe, is_final):
                ...
                if is_final:
                    tee_logger.info("\n" + "="*70)
                    tee_logger.info("✅ 所有季线分析脚本执行完成，PDF报告已合并")
