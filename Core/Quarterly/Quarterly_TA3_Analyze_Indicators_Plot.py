...
"""
Name: Quarterly_TA3_Analyze_Indicators_Plot.py
Function:
生成季线技术分析报告。
输入数据：SQLite数据库 (hk_quarterly_kline_analysis表)
输出数据文件：{ticker}_quarterly_TA_analysis.pdf
"""
...
from datetime import datetime, timedelta  # 日期时间对象处理
...
# 抑制matplotlib和PIL的日志输出
logging.getLogger('matplotlib').setLevel(logging.WARNING)
# 中文字体（如 SimHei）通常没有 bold 字重，matplotlib 会打印
# "Failed to find font weight bold, now using 400" 警告（回退到常规字重，不影响出图），
# 将该记录器级别提高到 ERROR 以消除这条无害提示
...
def setup_logger(log_dir):
    """配置日志系统 - 每次运行覆盖原有日志文件"""
    log_file = os.path.join(log_dir, 'Quarterly_TA3_Analyze_Indicators_Plot.log')
...
def load_data_from_db(ticker, start_date=None, end_date=None):
    ...
    try:
        ...
        query = """
        SELECT * FROM hk_quarterly_kline_analysis 
        WHERE stock_code = ?
        """
        params = [ticker]
...
def plot_quarterly_chart(ticker, output_pdf, start_date=None, end_date=None):
    """
    从数据库读取数据并绘制季线图表，包含RSI区域和技术指标状态表，结果保存为PDF

    参数:
    ticker: 股票代码
    output_pdf: 输出PDF文件路径
    start_date: 可选的开始日期 (YYYY-MM-DD格式)，若为None则动态计算（20年前或最早日期）
    end_date: 可选结束日期 (YYYY-MM-DD格式)，默认为最新交易日
    """
    try:
        logger = get_logger()
        logger.info(f"开始生成 {ticker} 季线图表")
        ...
        # 3. 动态计算起始日期：20年前的当季第1天 vs 数据最早日期
        twenty_years_ago = end_date - pd.DateOffset(years=20)
        computed_start = twenty_years_ago.to_period('Q').start_time
        start_date = max(computed_start, df['date'].min())
        logger.info(f"动态计算起始日期: {start_date.date()} (20年前当季: {computed_start.date()}, 数据最早: {df['date'].min().date()})")
        ...
        with PdfPages(output_pdf) as pdf:
            ...
            ax1.set_title(f'{ticker} 季线图 - {date_range_str}', fontsize=24)
            ...
            if 'vol_ema5' in df.columns:
                ax2.plot(df['date_num'], df['vol_ema5'], color='gold', linewidth=3, label='5个季度成交量均线')
            ...
            if ema20_break_above:
                table_data.append(["确认突破EMA20", "TRUE", 
                                  f"最近10个季度连续3个季度高于EMA20（起始日：{start_date_above.strftime('%Y-%m-%d')}）"])
            ...
            if ema20_break_below:
                table_data.append(["确认跌破EMA20", "TRUE", 
                                  f"最近10个季度连续3个季度低于EMA20（起始日：{start_date_below.strftime('%Y-%m-%d')}）"])
                streak_values.append(None)
            ...
            if ema200_break_above:
                table_data.append(["确认突破EMA200", "TRUE", 
                                  f"最近10个季度连续3个季度高于EMA200（起始日：{start_date_200_above.strftime('%Y-%m-%d')}）"])
                streak_values.append(None)
            ...
            if ema200_break_below:
                table_data.append(["确认跌破EMA200", "TRUE", 
                                  f"最近10个季度连续3个季度低于EMA200（起始日：{start_date_200_below.strftime('%Y-%m-%d')}）"])
                streak_values.append(None)

            # 序列号9: 季线EMA5/10/20黄金三角形
            if len(df) > 0 and 'golden_triangle' in df.columns:
                ...
                if not golden_true.empty:
                    ...
                    delta_quarters = (last_date.to_period('Q') - first_golden_date.to_period('Q')).n
                    table_data.append([
                        "EMA5/10/20黄金三角形", 
                        "TRUE", 
                        f"{first_golden_date.strftime('%Y-%m-%d')}，已出现{delta_quarters}个季度"
                    ])
                    streak_values.append(delta_quarters)

            # 序列号10: 季线EMA5/10/20死亡三角形
            if len(df) > 0 and 'death_triangle' in df.columns:
                ...
                if not death_true.empty:
                    ...
                    delta_quarters = (last_date.to_period('Q') - first_death_date.to_period('Q')).n
                    table_data.append([
                        "EMA5/10/20死亡三角形", 
                        "TRUE", 
                        f"{first_death_date.strftime('%Y-%m-%d')}，已出现{delta_quarters}个季度"
                    ])
                    streak_values.append(delta_quarters)
            ...
            if ema20_up_yang is not None:
                delta_quarters = (last_date.to_period('Q') - ema20_up_yang.to_period('Q')).n
                table_data.append([
                    "EMA20上方阳线", 
                    "TRUE", 
                    f"{ema20_up_yang.strftime('%Y-%m-%d')}，已出现{delta_quarters}个季度，累计{ema20_up_yang_count}次"
                ])
                streak_values.append(None)
            ...
            if ema20_down_yin is not None:
                delta_quarters = (last_date.to_period('Q') - ema20_down_yin.to_period('Q')).n
                table_data.append([
                    "EMA20下方阴线", 
                    "TRUE", 
                    f"{ema20_down_yin.strftime('%Y-%m-%d')}，已出现{delta_quarters}个季度，累计{ema20_down_yin_count}次"
                ])
                streak_values.append(None)
            ...
            if up_red > 0:
                table_data.append([
                    "跳空高开阳线", 
                    "TRUE", 
                    f"10个季度数量: {up_red_count}，最近出现: {df['date'].iloc[-up_red].strftime('%Y-%m-%d')}"
                ])
                streak_values.append(None)
            ...
            if down_black > 0:
                table_data.append([
                    "跳空低开阴线", 
                    "TRUE", 
                    f"10个季度数量: {down_black_count}，最近出现: {df['date'].iloc[-down_black].strftime('%Y-%m-%d')}"
                ])
                streak_values.append(None)
            ...
            if has_kdj:
                ...
                for i in range(1, len(lookback_window)):
                    ...
                    if curr['kdj_j'] <= 30 and prev['kdj_k'] < prev['kdj_d'] and curr['kdj_k'] > curr['kdj_d']:
                        kdj_golden = {'date': curr['date'], 'quarters': (df['date'].iloc[-1].to_period('Q') - curr['date'].to_period('Q')).n}
                        break
                    if curr['kdj_j'] >= 70 and prev['kdj_k'] > prev['kdj_d'] and curr['kdj_k'] < curr['kdj_d']:
                        kdj_death = {'date': curr['date'], 'quarters': (df['date'].iloc[-1].to_period('Q') - curr['date'].to_period('Q')).n}
                        break
            if kdj_golden:
                table_data.append([
                    "KDJ线金叉", "TRUE", 
                    f"金叉日期: {kdj_golden['date'].strftime('%Y-%m-%d')}, 持续季度数: {kdj_golden['quarters']}"
                ])
                streak_values.append(kdj_golden['quarters'])
            if kdj_death:
                table_data.append([
                    "KDJ线死叉", "TRUE", 
                    f"死叉日期: {kdj_death['date'].strftime('%Y-%m-%d')}, 持续季度数: {kdj_death['quarters']}"
                ])
                streak_values.append(kdj_death['quarters'])

            # 序列号18: MACD（修复显示持续季度数，兼容中英文状态）
            ...
            if is_golden:
                streak_quarters = last_row.get('macd_golden_streak', 0)
                if pd.notna(streak_quarters):
                    macd_details += f" / 金叉持续: {int(streak_quarters)}个季度"
                else:
                    macd_details += " / 金叉"
            elif is_death:
                streak_quarters = last_row.get('macd_death_streak', 0)
                if pd.notna(streak_quarters):
                    macd_details += f" / 死叉持续: {int(streak_quarters)}个季度"
                else:
                    macd_details += " / 死叉"
            else:
                macd_details = macd_value
            ...
            streak_values.append(streak_quarters if (is_golden or is_death) else None)
            ...
            # ========== 计算 EMA5 趋势（含连续三个季度 r 值、上行、下行、横摆、蓄势待发、弱势横摆、拐点识别）==========
                ...
                if len(r_vals) == 3:
                    ...
                    if all(r >= 1.0 for r in r_vals):
                        ...
                        if all(r > 0 for r in r_vals) and close_last > ema5_last:
                            ema5_trend = {
                                'status': '蓄势待发',
                                'detail': f"连续3个季度 r: [{r_str}]，均>0，收盘价{close_last:.2f}站上EMA5，蓄势上攻",
                                'color': '#CCFFCC'   # 浅绿
                            }
                        elif all(r < 0 for r in r_vals) and close_last < ema5_last:
                            ema5_trend = {
                                'status': '弱势横摆',
                                'detail': f"连续3个季度 r: [{r_str}]，均<0，收盘价跌破EMA5，弱势整理",
                                'color': '#E0E0E0'   # 浅灰
                            }
                        else:
                            ema5_trend = {
                                'status': '横摆',
                                'detail': f"连续3个季度 r: [{r_str}] (-1%~1%)，方向不一",
                                'color': None
                            }
                    else:
                        ...
                        if all(r > 0 for r in r_vals):
                            ema5_trend = {
                                'status': '温和上行',
                                'detail': f"连续3个季度 r: [{r_str}] (均>0，但未达1.0%阈值)，温和上涨趋势",
                                'color': '#C8E6C9'   # 浅绿色
                            }
                        # 3.2 温和下行：所有r<0，但至少有一个>-1.0%（即不完全满足严格下行）
                            ...
                            ema5_trend = {
                                'status': '温和下行',
                                'detail': f"连续3个季度 r: [{r_str}] (均<0，但未达-1.0%阈值)，温和下跌趋势",
                                'color': '#FFE0B2'   # 浅橙色
                            }
                        # 3.3 识别拐点（最近一个季度收于EMA5上方、r_last > 0、且前两个季度至少有一个为负）
                        elif close_last > ema5_last and r_last > 0 and any(r < 0 for r in r_vals[:-1]):
                            ema5_trend = {
                                'status': '拐点向上',
                                'detail': f"最近3个季度 r: [{r_str}]，收盘价{close_last:.2f}站上EMA5，季度增长率由负转正",
                                'color': '#90EE90'   # 绿色（与上行一致）
                            }
                        # 拐点向下：最近一个季度收于EMA5下方、r_last < 0、且前两个季度至少有一个为正
                        elif close_last < ema5_last and r_last < 0 and any(r > 0 for r in r_vals[:-1]):
                            ema5_trend = {
                                'status': '拐点向下',
                                'detail': f"最近3个季度 r: [{r_str}]，收盘价跌破EMA5，季度增长率由正转负",
                                'color': '#FFFF99'   # 黄色（与下行一致）
                            }
                        else:
                            ema5_trend = {
                                'status': '趋势不明',
                                'detail': f"最近3个季度 r: [{r_str}]，无明确连续信号",
                                'color': None
                            }
                else:
                    # 数据不足3个季度或存在缺失
                    ema5_trend = {
                        'status': 'N/A',
                        'detail': f"可用数据{len(last_3)}个季度，r: [{r_str}]，无法判断连续趋势",
                        'color': None
                    }
            else:
                ema5_trend = {
                    'status': 'N/A',
                    'detail': 'EMA5列缺失或数据不足3个季度',
                    'color': None
                }
            ...
            table.scale(1, 1.8)
            plt.suptitle(f'{ticker} 季线图技术指标状态 {date_range_str}', fontsize=24, y=0.95)
...
def main():
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("Quarterly_TA3_Analyze_Indicators_Plot 启动")
        logger.info("=" * 60)
        ...
        # 起始日期：20年前的当季第1天
        start_ts = (pd.Timestamp(today) - pd.DateOffset(years=20)).to_period('Q').start_time
        start_date = start_ts.date()

        # 截止日期：根据规则计算（季度版）
        if today.month in (3, 6, 9, 12) and today.day >= 25:
            # 当季最后一个自然日
            end_date = (pd.Timestamp(today).to_period('Q')).end_time.date()
            logger.info(f"当前日期 {today} 处于季度末月且 >= 25日，截止日期设为当季最后一个自然日：{end_date}")
        else:
            # 上季度最后一个自然日
            end_date = (pd.Timestamp(today).to_period('Q') - 1).end_time.date()
            logger.info(f"当前日期 {today}，截止日期设为上季度最后一个自然日：{end_date}")

        logger.info(f"📅 动态计算的日期范围:")
        ...
        if not tickers:
            ...
            logger.info(f"{current_time} 开始分析季技术分析指标...")
            ...
            for ticker in tickers:
                current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                logger.info(f"\n{current_time} 正在分析 {ticker} 的季线数据...")

                try:
                    # 生成PDF报告
                    output_pdf_file = os.path.join(GlobalConfig.full_report_dir, f"{ticker}_quarterly_TA_analysis.pdf")
                    plot_quarterly_chart(ticker, output_pdf_file, start_date, end_date)
                    
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.info(f"{current_time} {ticker} 季线分析完成")
                    success_count += 1
            ...
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            logger.info(f"{current_time} ✅ 全部季技术分析指标分析完成！")
