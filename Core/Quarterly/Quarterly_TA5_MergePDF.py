...
"""
Name: Quarterly_TA5_MergePDF.py
Function:
将下载
1. 合并季线技术分析与资金流分析图表文件.
2. 输入数据文件: 
    1) 季线技术分析图表文件{ticker}_quarterly_TA_analysis.pdf
    2) 季线资金流分析图表文件{ticker}_quarterly_moneyflow_analysis.pdf
3. 输出数据文件: 
    1) 季线资金流技术指标文件：{ticker}_quarterly_analysis_report.pdf         
"""
...
def setup_logger(log_dir):
    ...
    log_file = os.path.join(log_dir, 'Quarterly_TA5_MergePDF.log')
    
    # 创建日志目录（如果不存在）
    os.makedirs(log_dir, exist_ok=True)
    
    # 配置日志
    logger = logging.getLogger('Quarterly_TA5_MergePDF')
...
def load_stock_list_from_json(json_path, logger=None):
    """
    从 stock_list.json 文件加载股票列表（兼容两种字段名格式）
...
def main():
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("Quarterly_TA5_MergePDF 开始执行")
        logger.info("=" * 60)
        ...
        logger.info(f"{current_time} 开始合并季度分析报告！")
        print(f"{current_time} 开始合并季度分析报告！")
        ...
        for ticker in tickers:
            try:
                ...
                # 构建输入输出文件路径
                TA_pdf = os.path.join(GlobalConfig.full_report_dir, f'{ticker}_quarterly_TA_analysis.pdf')
                money_flow_pdf = os.path.join(GlobalConfig.full_report_dir, f'{ticker}_quarterly_moneyflow_analysis.pdf')

                input_files = [TA_pdf, money_flow_pdf]
                output_file = os.path.join(GlobalConfig.full_report_dir, f'{ticker}_quarterly_analysis_report.pdf')
        ...
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        logger.info(f"{current_time} ✅ 全部季度分析报告合并完成！")
        print(f"{current_time} ✅ 全部季度分析报告合并完成！")

        logger.info("=" * 60)
        logger.info("Quarterly_TA5_MergePDF 执行完成")
        logger.info("=" * 60)
