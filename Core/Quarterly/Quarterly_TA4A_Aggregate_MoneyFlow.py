...
"""
Name: Quarterly_TA4A_Aggregate_MoneyFlow.py
Function:
季线资金流数据聚合处理
输入数据表：hk_hist_daily_moneyflow
输出数据表：hk_hist_quarterly_moneyflow
"""
...
def setup_logging(log_dir: Path) -> logging.Logger:
    ...
    log_file = log_dir / 'Quarterly_TA4A_Aggregate_MoneyFlow.log'
    
    # 创建日志记录器
    logger = logging.getLogger('Quarterly_TA4A_Aggregate_MoneyFlow')
...
def get_cutoff_date(today: datetime, all_dates: list, logger: logging.Logger) -> pd.Timestamp:
    ...
    规则：
        - 若 today 处于季度末月（3/6/9/12月）且 day >= 25：取当前季度最后一个交易日
        - 否则：取上一季度最后一个交易日
    ...
    if today_date.month in (3, 6, 9, 12) and today_date.day >= 25:
        # 当前季度
    ...
    candidates = [d for d in all_dates_ts if d <= quarter_end]
    if candidates:
        cutoff_date = max(candidates)
        logger.info(f"当天日期 {today_date}，截止日期设为{period_desc}最后一个交易日: {cutoff_date.date()}")
    else:
        cutoff_date = max(all_dates_ts)
        logger.warning(f"未找到{period_desc}（截止{quarter_end.date()}）的交易日，使用全部数据最新日期: {cutoff_date.date()}")
...
def check_and_process_stock(conn: sqlite3.Connection, stock_code: str, stock_name: str, 
    ...
    try:
        ...
        cursor.execute("""
            SELECT name FROM sqlite_master 
            WHERE type='table' AND name='hk_hist_quarterly_moneyflow'
        """)
        if not cursor.fetchone():
            # 创建季线资金流数据表
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS hk_hist_quarterly_moneyflow (
            ...
            """)
            conn.commit()
            logger.info(f"创建季线资金流数据表 hk_hist_quarterly_moneyflow")
        ...
        if result and result['latest_date'] is not None:
            quarterly_latest_date = pd.to_datetime(result['latest_date'])
            logger.info(f"[{stock_code} {stock_name}] 整合数据最新日期: {quarterly_latest_date.date()}")
        else:
            quarterly_latest_date = None
            logger.info(f"[{stock_code} {stock_name}] 整合数据表中无该股票数据")
        
        # 5. 比较日期并决定是否计算
        if quarterly_latest_date is None or quarterly_latest_date < daily_latest_date:
            # 需要计算
            logger.info(f"[{stock_code} {stock_name}] 开始整合季线资金流数据...")
            aggregate_quarterly_money_flow(conn, stock_code, stock_name, daily_latest_date, 
                                        quarterly_latest_date, logger)
        elif quarterly_latest_date == daily_latest_date:
            logger.info(f"[{stock_code} {stock_name}] 资金流整合数据已存在，无需重复计算")
        elif quarterly_latest_date > daily_latest_date:
            logger.warning(f"[{stock_code} {stock_name}] 资金流原始数据缺失或季线分析数据有误，需进行核查")
...
def aggregate_quarterly_money_flow(conn: sqlite3.Connection, stock_code: str, stock_name: str,
                                 daily_latest_date: pd.Timestamp, quarterly_latest_date: pd.Timestamp,
                                 logger: logging.Logger) -> None:
    """
    将每日资金流数据按季度聚合，季度数据日期表示为该季度最后一个交易日
    
    根据当前日期与25日的比较，动态截取数据到上季末或当季末最后一个交易日
    ...
    Args:
        ...
        quarterly_latest_date: 整合数据最新日期（可能为None）
        logger: 日志记录器
    """
    try:
        # 1. 读取该股票的每日资金流数据
        if quarterly_latest_date is not None:
            query = """
                ...
                ORDER BY date ASC
            """
            df = pd.read_sql_query(query, conn, params=(stock_code, quarterly_latest_date.strftime('%Y-%m-%d')))
        else:
            query = """
                SELECT stock_code, stock_name, date, price_chgpct, capital_trend, 
                       extra_large, large, medium, small
        ...
        # 6. 定义聚合函数 - 使用该季度最后一个交易日作为日期
        def aggregate_quarter(group):
            """
            聚合一个季度的数据，日期使用该季度最后一个交易日
            """
            # 按日期排序，确保最后一个交易日是最大的日期
            group_sorted = group.sort_values('date')
            
            result = {
                'date': group_sorted['date'].iloc[-1],  # 使用该季度最后一个交易日
        ...
        # 7. 执行聚合
        quarterly = df.groupby('quarter_start', group_keys=False).apply(aggregate_quarter, include_groups=False).reset_index(drop=True)
        quarterly = quarterly.sort_values('date')
        
        logger.info(f"[{stock_code} {stock_name}] 聚合后得到 {len(quarterly)} 条季线数据")
        
        # 8. 过滤出需要插入的新数据
        if quarterly_latest_date is not None:
            # 只插入日期大于 quarterly_latest_date 的数据
            new_quarterly = quarterly[quarterly['date'] > quarterly_latest_date]
            logger.info(f"[{stock_code} {stock_name}] 其中新增 {len(new_quarterly)} 条季线数据需要插入")
        else:
            new_quarterly = quarterly
            logger.info(f"[{stock_code} {stock_name}] 首次处理，需要插入全部 {len(new_quarterly)} 条季线数据")
        
        if new_quarterly.empty:
            logger.info(f"[{stock_code} {stock_name}] 没有新的季线数据需要插入")
            return
        
        # 9. 插入新的整合数据
        cursor = conn.cursor()
        inserted_count = 0
        for _, row in new_quarterly.iterrows():
            try:
                cursor.execute("""
                    INSERT INTO hk_hist_quarterly_moneyflow 
        ...
        conn.commit()
        logger.info(f"[{stock_code} {stock_name}] 季线资金流数据整合完成，成功插入 {inserted_count} 条记录")
...
def main():
    ...
    try:
        ...
        # ========== 打印配置信息 ==========
        logger.info("=" * 60)
        logger.info("Quarterly_TA4A_Aggregate_MoneyFlow 启动")
        logger.info("=" * 60)
        ...
        try:
            ...
            logger.info(f"{current_time} 开始整合季线资金流数据...")
            ...
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            logger.info("=" * 60)
            logger.info(f"{current_time} ✅ 全部季线资金流数据整合完成！")
            logger.info("=" * 60)
            logger.info(f"📊 处理统计:")
