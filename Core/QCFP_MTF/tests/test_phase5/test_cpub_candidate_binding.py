...
def test_cbind_t02_manifest_sha_matches_bytes():
    ...
    for entry in manifest["files"]:
        # Current manifest schema carries role/materialization/sha256 per entry.
        assert entry.get("role")
        assert entry.get("sha256")
        assert entry.get("materialization_type")
        path = PROJECT_ROOT / entry["candidate_path"]
        assert path.exists()
        if entry.get("bytes") is not None:
            assert path.stat().st_size == entry["bytes"]
        if entry["materialization_type"] == \
...
def test_cbind_t03_package_references_exact_candidate_sha():
    ...
    for key, path in package["referenced_files"].items():
        if key == "GOVERNANCE_BASELINE_PREACTIVATION_CANDIDATE":
            # Pre-transition mirror identity is preserved in the immutable
            # predecessor archive; the ACTIVE path now carries the successor.
            actual = _sha(
                PROJECT_ROOT / "audit/phase5/external_governance_candidates/"
                "r2_15f/chain_a/predecessor_baseline_163ddd73.json")
        else:
    ...
    for entry in manifest["files"]:
        ...
        if materialization_type == "EXACT_MATERIALIZATION_TARGET":
            ...
            if target == "audit/phase5/phase5_governance_baseline.json":
                # ACTIVE baseline is the current successor (resolver-bound);
                # the manifest mirror identity is the pre-transition identity
                # and must remain distinct from the current active identity.
                assert _sha(path) == _ids["baseline"]
                assert entry["sha256"] != _ids["baseline"]
            else:
                # Frozen-surface manifest mirror equals ACTIVE manifest bytes.
                assert _sha(path) == entry["sha256"]
        else:
...
def test_baseline_candidate_is_full_pre_activation_revision():
    ...
    # Historical pre-activation mirror (05) stays immutable and is NOT the
    # current or immediate-predecessor baseline of the successor chain.
    assert candidate_sha != current_sha
    assert candidate["activation"] is False
...
def test_baseline_finalizer_deterministic_and_only_activation_fields():
    ...
    activation_bytes = builder.build_activation_record(
        ...
        human_acceptance_bytes=acceptance,
        candidate_manifest_bytes=manifest_bytes)
    predecessor_baseline = (PROJECT_ROOT /
        "audit/phase5/external_governance_candidates/r2_15f/chain_a/"
        "predecessor_baseline_163ddd73.json").read_bytes()
    b1 = finalizer.finalize_governance_baseline(
        candidate_baseline_bytes=predecessor_baseline,
        candidate_package_bytes=package_bytes,
        human_acceptance_bytes=acceptance,
