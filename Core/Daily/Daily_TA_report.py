...
SCRIPTS_DIR = os.path.join(PROJECT_ROOT, "Core", "Daily")
VENV_DIR = os.path.join(PROJECT_ROOT, ".venv")
LAUNCHER_PATH = os.path.join(PROJECT_ROOT,"Core", "Utl", "stock_analysis_utl.py")

# 日志文件路径配置
LOG_DIR = os.path.join(PROJECT_ROOT, "Log")
LOG_FILE = os.path.join(LOG_DIR, "Daily_TA_report.log")
...
class TeeLogger:
    ...
    def _setup_logging(self):
        ...
        # 设置logger
        self.logger = logging.getLogger('Daily_TA_Report')
...
def main():
    ...
    try:
        ...
        scripts = [
            ...
            "Daily_TA1C_download_institutional_holdings.py",
        ...
        tee_logger.info(f"\n{'='*70}")
        tee_logger.info(f"开始顺序执行 {len(scripts)} 个脚本")
        tee_logger.info(f"开始时间: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        for i, script in enumerate(scripts, 1):
            tee_logger.info(f"\n>>> 进度: {i}/{len(scripts)} <<<")
            if run_script(script, python_exe):
                success_count += 1
            else:
