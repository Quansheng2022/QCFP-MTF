...
"""
Name: Monthly_TA3_Analyze_Indicators_Plot.py
Function:
生成月线技术分析报告。
输入数据：SQLite数据库 (hk_monthly_kline_analysis表)
输出数据文件：{ticker}_monthly_TA_analysis.pdf
"""
...
def setup_logger(log_dir):
    """配置日志系统 - 每次运行覆盖原有日志文件"""
    log_file = os.path.join(log_dir, 'Monthly_TA3_Analyze_Indicators_Plot.log')
...
def load_data_from_db(ticker, start_date=None, end_date=None):
    ...
    try:
        ...
        query = """
        SELECT * FROM hk_monthly_kline_analysis 
        WHERE stock_code = ?
        """
        params = [ticker]
...
def plot_monthly_chart(ticker, output_pdf, start_date=None, end_date=None):
    """
    从数据库读取数据并绘制月线图表，包含RSI区域和技术指标状态表，结果保存为PDF

    参数:
    ticker: 股票代码
    output_pdf: 输出PDF文件路径
    start_date: 可选的开始日期 (YYYY-MM-DD格式)，若为None则动态计算（7年前或最早日期）
    end_date: 可选结束日期 (YYYY-MM-DD格式)，默认为最新交易日
    """
    try:
        logger = get_logger()
        logger.info(f"开始生成 {ticker} 月线图表")
        ...
        # 3. 动态计算起始日期：7年前的当月1日 vs 数据最早日期
        seven_years_ago = end_date - pd.DateOffset(years=7)
        computed_start = seven_years_ago.replace(day=1)
        start_date = max(computed_start, df['date'].min())
        logger.info(f"动态计算起始日期: {start_date.date()} (7年前: {computed_start.date()}, 数据最早: {df['date'].min().date()})")
        ...
        with PdfPages(output_pdf) as pdf:
            ...
            ax1.set_title(f'{ticker} 月线图 - {date_range_str}', fontsize=24)
            ...
            if 'vol_ema5' in df.columns:
                ax2.plot(df['date_num'], df['vol_ema5'], color='gold', linewidth=3, label='5个月成交量均线')
            ...
            if ema20_break_above:
                table_data.append(["确认突破EMA20", "TRUE", 
                                  f"最近10个月连续3个月高于EMA20（起始日：{start_date_above.strftime('%Y-%m-%d')}）"])
            ...
            if ema20_break_below:
                table_data.append(["确认跌破EMA20", "TRUE", 
                                  f"最近10个月连续3个月低于EMA20（起始日：{start_date_below.strftime('%Y-%m-%d')}）"])
                streak_values.append(None)
            ...
            if ema200_break_above:
                table_data.append(["确认突破EMA200", "TRUE", 
                                  f"最近10个月连续3个月高于EMA200（起始日：{start_date_200_above.strftime('%Y-%m-%d')}）"])
                streak_values.append(None)
            ...
            if ema200_break_below:
                table_data.append(["确认跌破EMA200", "TRUE", 
                                  f"最近10个月连续3个月低于EMA200（起始日：{start_date_200_below.strftime('%Y-%m-%d')}）"])
                streak_values.append(None)

            # 序列号9: 月线EMA5/10/20黄金三角形
            ...
            if up_red > 0:
                table_data.append([
                    "跳空高开阳线", 
                    "TRUE", 
                    f"10个月数量: {up_red_count}，最近出现: {df['date'].iloc[-up_red].strftime('%Y-%m-%d')}"
                ])
                streak_values.append(None)
            ...
            if down_black > 0:
                table_data.append([
                    "跳空低开阴线", 
                    "TRUE", 
                    f"10个月数量: {down_black_count}，最近出现: {df['date'].iloc[-down_black].strftime('%Y-%m-%d')}"
                ])
                streak_values.append(None)
            ...
            if is_golden:
                streak_months = last_row.get('macd_golden_streak', 0)
                if pd.notna(streak_months):
                    macd_details += f" / 金叉持续: {int(streak_months)}个月"
                else:
                    macd_details += " / 金叉"
            elif is_death:
                streak_months = last_row.get('macd_death_streak', 0)
                if pd.notna(streak_months):
                    macd_details += f" / 死叉持续: {int(streak_months)}个月"
                else:
                    macd_details += " / 死叉"
            else:
                macd_details = macd_value
            ...
            streak_values.append(streak_months if (is_golden or is_death) else None)
            ...
            # ========== 计算 EMA5 趋势（含连续三个月 r 值、上行、下行、横摆、蓄势待发、弱势横摆、拐点识别）==========
                ...
                if len(r_vals) == 3:
                    ...
                    if all(r >= 1.0 for r in r_vals):
                        ...
                        if all(r > 0 for r in r_vals) and close_last > ema5_last:
                            ema5_trend = {
                                'status': '蓄势待发',
                                'detail': f"连续3个月 r: [{r_str}]，均>0，收盘价{close_last:.2f}站上EMA5，蓄势上攻",
                                'color': '#CCFFCC'   # 浅绿
                            }
                        elif all(r < 0 for r in r_vals) and close_last < ema5_last:
                            ema5_trend = {
                                'status': '弱势横摆',
                                'detail': f"连续3个月 r: [{r_str}]，均<0，收盘价跌破EMA5，弱势整理",
                                'color': '#E0E0E0'   # 浅灰
                            }
                        else:
                            ema5_trend = {
                                'status': '横摆',
                                'detail': f"连续3个月 r: [{r_str}] (-1%~1%)，方向不一",
                                'color': None
                            }
                    else:
                        ...
                        if all(r > 0 for r in r_vals):
                            ema5_trend = {
                                'status': '温和上行',
                                'detail': f"连续3个月 r: [{r_str}] (均>0，但未达1.0%阈值)，温和上涨趋势",
                                'color': '#C8E6C9'   # 浅绿色
                            }
                        # 3.2 温和下行：所有r<0，但至少有一个>-1.0%（即不完全满足严格下行）
                            ...
                            ema5_trend = {
                                'status': '温和下行',
                                'detail': f"连续3个月 r: [{r_str}] (均<0，但未达-1.0%阈值)，温和下跌趋势",
                                'color': '#FFE0B2'   # 浅橙色
                            }
                        # 3.3 识别拐点（最近一个月收于EMA5上方、r_last > 0、且前两个月至少有一个为负）
                        elif close_last > ema5_last and r_last > 0 and any(r < 0 for r in r_vals[:-1]):
                            ema5_trend = {
                                'status': '拐点向上',
                                'detail': f"最近3个月 r: [{r_str}]，收盘价{close_last:.2f}站上EMA5，月增长率由负转正",
                                'color': '#90EE90'   # 绿色（与上行一致）
                            }
                        # 拐点向下：最近一个月收于EMA5下方、r_last < 0、且前两个月至少有一个为正
                        elif close_last < ema5_last and r_last < 0 and any(r > 0 for r in r_vals[:-1]):
                            ema5_trend = {
                                'status': '拐点向下',
                                'detail': f"最近3个月 r: [{r_str}]，收盘价跌破EMA5，月增长率由正转负",
                                'color': '#FFFF99'   # 黄色（与下行一致）
                            }
                        else:
                            ema5_trend = {
                                'status': '趋势不明',
                                'detail': f"最近3个月 r: [{r_str}]，无明确连续信号",
                                'color': None
                            }
                else:
                    # 数据不足3个月或存在缺失
                    ema5_trend = {
                        'status': 'N/A',
                        'detail': f"可用数据{len(last_3)}个月，r: [{r_str}]，无法判断连续趋势",
                        'color': None
                    }
            else:
                ema5_trend = {
                    'status': 'N/A',
                    'detail': 'EMA5列缺失或数据不足3个月',
                    'color': None
                }
            ...
            table.scale(1, 1.8)
            plt.suptitle(f'{ticker} 月线图技术指标状态 {date_range_str}', fontsize=24, y=0.95)
...
def main():
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("Monthly_TA3_Analyze_Indicators_Plot 启动")
        logger.info("=" * 60)
        ...
        start_date = today.replace(year=start_year, day=1)

        # 截止日期：根据规则计算
        if today.day < 25:
            # 上月最后一个自然日
            end_date = today.replace(day=1) - timedelta(days=1)
            logger.info(f"当前日期 {today} < 25，截止日期设为上月最后一个交易日（自然日：{end_date}）")
        else:
            # 当月最后一个自然日
            end_date = today.replace(day=1) + pd.offsets.MonthEnd(0)
            logger.info(f"当前日期 {today} >= 25，截止日期设为当月最后一个交易日（自然日：{end_date}）")
        ...
        if not tickers:
            ...
            logger.info(f"{current_time} 开始分析月技术分析指标...")
            ...
            for ticker in tickers:
                current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                logger.info(f"\n{current_time} 正在分析 {ticker} 的月线数据...")

                try:
                    # 生成PDF报告
                    output_pdf_file = os.path.join(GlobalConfig.full_report_dir, f"{ticker}_monthly_TA_analysis.pdf")
                    plot_monthly_chart(ticker, output_pdf_file, start_date, end_date)
                    
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.info(f"{current_time} {ticker} 月线分析完成")
                    success_count += 1
            ...
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            logger.info(f"{current_time} ✅ 全部月技术分析指标分析完成！")
