...
"""
Name: Monthly_TA2B_Calculate_indicators.py
Function: 
计算月线数据技术分析指标。
输入数据表：hk_hist_monthly_kline
输出数据表：hk_monthly_kline_analysis
"""
...
def setup_logging(log_dir):
    """设置日志配置 - 每次运行覆盖旧日志文件"""
    log_file = Path(log_dir) / 'Monthly_TA2B_Calculate_indicators.log'
...
def load_monthly_data_from_db(conn, stock_code):
    """从数据库加载月线数据"""
    query = """
        SELECT 
            date,
...
def save_analysis_to_db(conn, df, stock_code, stock_name):
    ...
    # 先删除该股票已有的分析数据
    cursor.execute("DELETE FROM hk_monthly_kline_analysis WHERE stock_code = ?", (stock_code,))
    ...
    if records:
        ...
        insert_sql = f"INSERT INTO hk_monthly_kline_analysis ({columns_str}) VALUES ({placeholders})"
...
def print_latest_analysis(df, stock_code, logger):
    ...
    info_lines = []
    info_lines.append(f"\n{'='*60}")
    info_lines.append(f"📊 股票 {stock_code} 最新月线分析结果")
        ...
        if status_col in df.columns and pd.notna(latest_month.get(status_col)):
            ...
            if status and status != '':
                ...
                if isinstance(streak, (int, float)):
                    ...
                    streak_int = 0
                ema_status_lines.append(f"{ema_pair_name}状态: {status} (已持续 {streak_int} 个月)")
    if ema_status_lines:
    ...
    if 'macd_status' in df.columns and pd.notna(latest_month.get('macd_status')):
        ...
        if macd_status and macd_status != '':
            ...
            if isinstance(streak, (int, float)):
                ...
                streak_int = 0
            info_lines.append(f"MACD状态: {macd_status} (已持续 {streak_int} 个月)")
    ...
    if pd.notna(k_val) and isinstance(k_val, (int, float)):
        ...
        info_lines.append("KDJ: 数据不足（需至少9个月数据）")
    ...
    if all(col in df.columns for col in ['volume', 'vol_ema5', 'volume_ratio']):
        if pd.notna(latest_month.get('volume')):
            ...
            if isinstance(volume_ratio, (int, float)):
                info_lines.append(f"成交量: {volume_msg} (比率: {volume_ratio:.2f}x) vs 5个月均量: {ma5_msg}")
            else:
                info_lines.append(f"成交量: {volume_msg} vs 5个月均量: {ma5_msg}")
...
def main():
    ...
    try:
        ...
        logger.info("="*70)
        logger.info("="*60)
        logger.info("Monthly_TA2B_Calculate_indicators 启动")
        logger.info("="*60)
        logger.info(f"项目目录: {project_dir}")
        logger.info(f"配置文件位置: {par_config_path}")
        logger.info(f"日志文件位置: {GlobalConfig.full_log_dir / 'Monthly_TA2B_Calculate_indicators.log'}")
        ...
        try:
            ...
            for stock_code in tickers:
                ...
                if status == 'up_to_date':
                    logger.info(f"✅ {stock_code} 月线分析数据已存在，无需重复计算")
                    skipped_stocks.append(stock_code)
                    # 加载已有分析数据用于显示
                    df = load_monthly_data_from_db(conn, stock_code)
                    if not df.empty:
                        print_latest_analysis(df, stock_code, logger)
                    continue
                
                elif status == 'data_error':
                    logger.warning(f"⚠️ {stock_code} 月线原始数据缺失或分析数据有误，需进行核查")
                    ...
                    # 进行分析
                    analyzed_df = analyze_monthly_data(df)
