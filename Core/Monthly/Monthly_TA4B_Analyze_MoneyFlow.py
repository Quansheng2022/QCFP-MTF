...
"""
Name: Monthly_TA4B_Analyze_MoneyFlow.py
Function:
1. 从SQLite数据库读取月线资金流数据
2. 计算生成各种月线资金流技术指标
3. 将计算结果保存到SQLite数据库
4. 生成月线资金流分析图表PDF
5. 使用并行处理进行优化

输入数据表：hk_hist_monthly_moneyflow 
输出数据表：hk_monthly_moneyflow_analysis 
生成月线资金流分析图表：{ticker}_monthly_moneyflow_analysis.pdf
...
def setup_logger(log_dir):
    ...
    """
    log_file = os.path.join(log_dir, 'Monthly_TA4B_Analyze_MoneyFlow.log')
...
def adjust_date_to_month_end(date_value):
    """
    将日期矫正为当月的最后一天
    ...
    Returns:
        str: 格式化为 'YYYY-MM-DD' 的月末日期字符串
    
    Examples:
        >>> adjust_date_to_month_end('2026-06-15')
        '2026-06-30'
        >>> adjust_date_to_month_end('2026-06-30')
        '2026-06-30'
    """
    # 转换为datetime对象
...
def adjust_date_column_to_month_end(df, date_column='date'):
    """
    将DataFrame中的日期列矫正为当月的最后一天
    ...
    # 应用日期矫正
    result_df[date_column] = result_df[date_column].apply(adjust_date_to_month_end)
    
    # 记录日志
    logger = get_logger()
    logger.debug(f"日期列 '{date_column}' 已矫正为当月最后一天")
    
    return result_df

def get_dynamic_cutoff_date() -> datetime.date:
    """
    根据运行当天日期返回截止日期（自然月末，近似最后一个交易日）：
    - 如果当天日期 < 25: 返回上个月最后一天（视为上月最后一个交易日）
    - 如果当天日期 >= 25: 返回当月最后一天（视为当月最后一个交易日）
    """
    today = datetime.now().date()
    first_of_this_month = today.replace(day=1)
    if today.day < 25:
        # 上月最后一天
        cutoff = first_of_this_month - timedelta(days=1)
        logger = get_logger()
        logger.info(f"当前日期 {today} < 25，截止日期设为上月最后一个交易日（近似）: {cutoff}")
    else:
        # 当月最后一天
        next_month_first = (first_of_this_month.replace(day=28) + timedelta(days=4)).replace(day=1)
        cutoff = next_month_first - timedelta(days=1)
        logger = get_logger()
        logger.info(f"当前日期 {today} >= 25，截止日期设为当月最后一个交易日（近似）: {cutoff}")
    return cutoff
...
def get_stock_name_from_db(stock_code):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT stock_name FROM hk_hist_monthly_moneyflow WHERE stock_code = ? LIMIT 1",
            (stock_code,)
        )
...
def save_analysis_to_db(df, stock_code):
    ...
    try:
        # ===== 关键修改：在保存前进行日期矫正 =====
        # 将日期矫正为当月最后一天
        df_corrected = adjust_date_column_to_month_end(df, 'date')
        
        logger = get_logger()
        logger.info(f"{stock_code}: 日期已矫正为当月最后一天")
        ...
        for _, row in df_corrected.iterrows():
            cursor.execute("""
                INSERT OR REPLACE INTO hk_monthly_moneyflow_analysis 
...
def check_and_process_stock(stock_code):
    ...
    """
    # 获取原始数据最新日期
    source_latest = get_latest_date_from_table('hk_hist_monthly_moneyflow', stock_code)
    if not source_latest:
        return False, f"{stock_code} 原始数据缺失", None, None
    
    # 获取分析数据最新日期
    analysis_latest = get_latest_date_from_table('hk_monthly_moneyflow_analysis', stock_code)
...
def calculate_money_flow_indicator_df(df):
    """
    计算月资金流指标（机构/散户资金流、5月均线、IDR、FBI等）
    注意：在计算前会进行日期矫正
    ...
    Returns:
        pd.DataFrame: 包含计算结果的DataFrame（日期已矫正为月末）
    """
    if df.empty:
        return df
    ...
    # ===== 关键修改：在计算指标前进行日期矫正 =====
    # 将日期矫正为当月最后一天
    result_df = adjust_date_column_to_month_end(result_df, 'date')
    ...
    logger = get_logger()
    logger.debug(f"日期已矫正为月末，共处理 {len(result_df)} 条记录")
...
def create_grouped_stacked_bars(current_ticker, date_range_label, period_data_cache, data_periods=None):
    """
    Optimized function to create grouped stacked bar charts with negative value support (Monthly version)
    """
    ...
    ax.set_title(
        f'{current_ticker} Investor Flow Breakdown Comparison (Monthly)',
        fontsize=16,
        pad=20
    )
...
def generate_moneyflow_pdf(stock_code, pdf_path, start_date=None, end_date=None):
    """
    生成月线资金流分析图表PDF（使用matplotlib的PDF后端）
    ...
    try:
        ...
        if len(df_latest) < 5:
            logger.warning(f"{stock_code}: 分析期间数据不足5个月 ({len(df_latest)}个月)")
...
资金流指标:
- 机构主导月数: {(inst_flow > ind_flow).sum()}/{len(df_latest)}
- IDR均值: {df_latest['IDR'].mean():.2f}, FBI均值: {df_latest['FBI'].mean():.2f}
- 平均月涨幅: {df_latest['Price_Chg%'].mean():.2f}%
"""
            ...
            ax1.set_title(f'{stock_code} ({stock_name}) 月资金流趋势\n{date_range_label}')
            ax1.set_xlabel('日期')
            ...
            ax2.plot(df_latest['Date'], df_latest['Inst_5MA']/scale, label='机构5月均线', color='#1f77b4')
            ax2.plot(df_latest['Date'], df_latest['Ind_5MA']/scale, label='散户5月均线', color='#ff7f0e')
            ax2.axhline(0, color='black', linestyle='--')
            ax2.set_title(f'{stock_code} ({stock_name}) 5月移动平均线\n{date_range_label}')
            ax2.set_xlabel('日期')
            ...
            for p in all_periods:
                if isinstance(p, int) and len(df) >= p:
                    period_cache[p] = (f"最新{p}个月", df.tail(p))
                elif p == 'AP':
...
def process_single_ticker(ticker, CONFIG, start_date=None, end_date=None):
    ...
    try:
        ...
        # 准备PDF路径
        pdf_path = os.path.join(GlobalConfig.full_report_dir, f"{ticker}_monthly_moneyflow_analysis.pdf")
        ...
        existing_df = pd.read_sql_query(
            "SELECT * FROM hk_monthly_moneyflow_analysis WHERE stock_code = ?",
            conn, params=(ticker,)
        )
        conn.close()
...
def main():
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("Monthly_TA4B_Analyze_MoneyFlow 启动")
        logger.info("=" * 80)
        ...
        # ========== 动态计算起止日期 ==========
        # 1. 根据当天日期计算截止日期（月末近似交易日）
        end_date = get_dynamic_cutoff_date()
        
        # 2. 起始日期为截止日期前推7年的当月1日
        start_year = end_date.year - 7
        start_month = end_date.month
        start_date = datetime(start_year, start_month, 1).date()
        
        logger.info(f"\n📅 月线资金流分析的日期范围（动态计算）:")
        logger.info(f"   起始日期: {start_date.strftime('%Y-%m-%d')} (7年前的{start_month}月1日)")
        logger.info(f"   截止日期: {end_date.strftime('%Y-%m-%d')} (根据当天日期动态确定)")
        logger.info(f"   注意: 如果某只股票数据不足7年，分析时将自动使用其最早可用日期作为起始日期。")
        ...
        logger.info(f"{current_time} 开始分析月资金流指标...")
