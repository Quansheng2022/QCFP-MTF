...
# P1-1（SOURCE-BUNDLE-V1.1）：Sidecar Manifest 的正式 Schema 常量。
# 与 BUNDLE_SCHEMA 是两回事：Bundle 文本 vs Sidecar JSON。
SIDECAR_SCHEMA = "QCFP-SOURCE-BUNDLE-MANIFEST-2"
...
def canonical_profile_targets(profile: str) -> list[str]:
    """P1-1：返回 Profile 的规范化 Canonical Scan Targets。"""
    return normalize_scan_targets(profile_scan_targets(profile))
...
def validate_profile_scope(
    ...
    """P1-1：Profile ↔ Scope 严格 1:1（完全相等，不是 subset）。

    同时阻止 Scope Narrowing（A+B）与 Scope Expansion（A+B+secret）。
    """
    try:
        expected = canonical_profile_targets(profile)
    except ValueError as exc:
        return False, str(exc)

    actual = normalize_scan_targets(scan_targets)

    if actual != expected:
        return (
            False,
            "SCAN_TARGETS_DO_NOT_MATCH_PROFILE: "
            f"profile={profile!r}, "
...
def write_bundle(
    ...
    # P1-2：Builder 强制 Profile ↔ Scope Hard Binding（API 也必须安全）
    scope_ok, scope_reason = validate_profile_scope(profile, scan_targets)
    if not scope_ok:
...
def parse_int_field(
    ...
    """P1-5：整数读取；Missing/Malformed 一律进 problems（安全边界 FAIL）。"""
    value = mapping.get(key)
    try:
        return int(value)
    except (TypeError, ValueError):
        problems.append(f"{key} 非法或缺失: {value!r}")
        return None
...
def verify_sidecar_identity(
    ...
    """P1-7：Sidecar 与 Header/Footer 的身份字段双向一致（含 Manifest Schema）。"""
    problems: list[str] = []
    expected_pairs = {
        "bundle_schema": footer.get("BUNDLE_SCHEMA"),
        "bundle_profile": header.get("bundle_profile"),
        "bundle_status": footer.get("BUNDLE_STATUS"),
        "scope_manifest_sha256": footer.get("SCOPE_MANIFEST_SHA256"),
        "manifest_sha256": footer.get("MANIFEST_SHA256"),
        "bundle_root_sha256": footer.get("BUNDLE_ROOT_SHA256"),
    }
    for key, expected in expected_pairs.items():
        actual = sidecar.get(key)
        if actual != expected:
            problems.append(
                f"Sidecar identity mismatch: {key}: "
                f"sidecar={actual!r}, bundle={expected!r}"
            )
    if sidecar.get("schema") != SIDECAR_SCHEMA:
        problems.append(
            "Sidecar manifest schema mismatch: "
            f"actual={sidecar.get('schema')!r}, "
            f"expected={SIDECAR_SCHEMA!r}"
        )
    return problems


def sidecar_list_count_matches(
    sidecar: dict,
    list_key: str,
    expected_count: Optional[int],
) -> bool:
    """P1-6：Sidecar 错误/排除列表长度必须等于对应 count。"""
    value = sidecar.get(list_key)
    return (
...
def verify_bundle(
    ...
    """P1 Verifier Identity Binding（离线，不依赖项目源目录）。

    固定验证顺序（Actual Data → Calculated Hash → Declared Hash）：
        V01-V05  存在性/解析
        V10-V16  Footer / Bundle Schema 三方绑定 / Sidecar Manifest Schema /
                 Canonical Profile / Profile Binding / Sidecar Scan Targets /
                 Footer Invariants
        V20-V23  Boundary / Unique Paths / File Count / Content
        V30-V32  Calculated Manifest → Footer → Sidecar
        V40-V42  Calculated Scope → Footer → Sidecar
        V50-V52  Calculated Root（用计算值）→ Footer → Sidecar
        V60-V66  Sidecar Identity / Records / File Count / Error Counts /
                 Error Lists / Status / Strict Mode
        V70      Whole Bundle SHA256
    """
    checks: dict[str, str] = {}
    problems: list[str] = []
    ...
    # V04 Bundle UTF-8 decode
    raw = bundle_path.read_bytes()
    try:
        text = raw.decode("utf-8")
    except UnicodeDecodeError as exc:
        return {"verdict": "BUNDLE_VERIFICATION_FAILED",
                "checks": {"Bundle decode": "FAIL"},
                "problems": [f"bundle 非 UTF-8: {exc}"]}
    # V05 Bundle parse
    ...
    # V10 Footer exists
    checks["Footer exists"] = "PASS" if "BUNDLE_SCHEMA" in footer else "FAIL"
    if checks["Footer exists"] == "FAIL":
        problems.append("footer 缺失")
    # V11 Bundle Schema Binding（Header == Footer == Sidecar == BUNDLE_SCHEMA）
    header_schema = header.get("bundle_schema")
    footer_schema = footer.get("BUNDLE_SCHEMA")
    sidecar_bundle_schema = sidecar.get("bundle_schema")
    schema_ok = (
        header_schema == BUNDLE_SCHEMA
        and footer_schema == BUNDLE_SCHEMA
        and sidecar_bundle_schema == BUNDLE_SCHEMA
    )
    checks["Schema"] = "PASS" if schema_ok else "FAIL"
    if not schema_ok:
        problems.append(
            "Bundle Schema 不一致："
            f"header={header_schema!r}, footer={footer_schema!r}, "
            f"sidecar={sidecar_bundle_schema!r}, "
            f"expected={BUNDLE_SCHEMA!r}"
        )
    # V12 Sidecar Manifest Schema（sidecar.schema 自己的权威）
    sidecar_schema_ok = sidecar.get("schema") == SIDECAR_SCHEMA
    checks["Sidecar Manifest Schema"] = "PASS" if sidecar_schema_ok \
        else "FAIL"
    if not sidecar_schema_ok:
        problems.append(
            "Sidecar schema 不匹配："
            f"actual={sidecar.get('schema')!r}, "
            f"expected={SIDECAR_SCHEMA!r}"
        )
    # V13 Canonical Profile：先验证 Profile Authority，再信 Hash
    profile = header.get("bundle_profile", "")
    scan_targets = header.get("scan_targets") or []
    profile_ok, profile_reason = validate_profile_scope(profile, scan_targets)
    checks["Canonical Profile"] = "PASS" if profile_ok else "FAIL"
    if not profile_ok:
        problems.append(profile_reason)
    # V14 Profile Binding：Header == Footer
    footer_profile_ok = footer.get("BUNDLE_PROFILE") == profile
    checks["Profile Binding"] = "PASS" if footer_profile_ok else "FAIL"
    if not footer_profile_ok:
        problems.append(
            "Header bundle_profile 与 Footer BUNDLE_PROFILE 不一致"
        )
    # V15 Sidecar Scan Targets：Sidecar == Header == Canonical Profile
    sidecar_targets = sidecar.get("scan_targets")
    canonical_targets = canonical_profile_targets(profile) if profile_ok \
        else None
    sidecar_targets_ok = (
        isinstance(sidecar_targets, list)
        and normalize_scan_targets(sidecar_targets)
        == normalize_scan_targets(scan_targets)
        and normalize_scan_targets(sidecar_targets)
        == canonical_targets
    )
    checks["Sidecar Scan Targets"] = "PASS" if sidecar_targets_ok \
        else "FAIL"
    if not sidecar_targets_ok:
        problems.append(
            "Sidecar scan_targets 与 Bundle/Canonical Profile 不一致"
        )
    # V16 Footer Invariants：COMPLETE 要求 scan/merge/integrity 全 0
    footer_merged = parse_int_field(footer, "MERGED_FILES", problems)
    footer_scan_errors = parse_int_field(footer, "SCAN_ERRORS", problems)
    footer_merge_errors = parse_int_field(footer, "MERGE_ERRORS", problems)
    footer_integrity_errors = parse_int_field(
        footer, "INTEGRITY_ERRORS", problems)
    footer_security_exclusions = parse_int_field(
        footer, "SECURITY_EXCLUSIONS", problems)
    footer_complete_ok = (
        footer.get("BUNDLE_STATUS") == "COMPLETE"
        and footer.get("QCFP_BUNDLE_COMPLETE") == "TRUE"
        and footer_scan_errors == 0
        and footer_merge_errors == 0
        and footer_integrity_errors == 0
    )
    checks["Footer Invariants"] = "PASS" if footer_complete_ok else "FAIL"
    if not footer_complete_ok:
        problems.append("Footer 不变量不成立：errors 非 0 却声明 COMPLETE")

    # V20 Boundary Integrity
    ...
    # V21 Unique Paths
    unique_paths = len({r.path for r in records}) == len(records)
    checks["Unique Paths"] = "PASS" if unique_paths else "FAIL"
    if not unique_paths:
        problems.append("存在重复 path")
    # V22 File Count：records == Footer MERGED_FILES
    actual_count = len(records)
    count_ok = footer_merged is not None and actual_count == footer_merged
    checks["File Count"] = (
        f"{actual_count}/{footer_merged}"
        if footer_merged is not None
        else f"{actual_count}/INVALID"
    )
    if not count_ok:
        problems.append("Bundle records 数量与 Footer MERGED_FILES 不一致")
    # V23 Content Hash / char / line
    content_ok = 0
    for record in records:
    ...
    # V30-V32 Calculated Manifest → Footer → Sidecar
    calculated_manifest = manifest_hash(records)
    checks["Manifest Hash"] = "PASS" if calculated_manifest == \
        footer.get("MANIFEST_SHA256") else "FAIL"
    if checks["Manifest Hash"] == "FAIL":
        problems.append("Calculated MANIFEST_SHA256 与 Footer 不一致")
    sidecar_manifest_ok = sidecar.get("manifest_sha256") == \
        calculated_manifest
    if not sidecar_manifest_ok:
        problems.append("Sidecar manifest_sha256 与 Calculated 不一致")

    # V40-V42 Calculated Scope（仅当 Canonical Profile 成立后计算）→ Footer → Sidecar
    calculated_scope = scope_manifest_hash(profile, scan_targets) \
        if profile_ok else ""
    checks["Scope Hash"] = "PASS" if profile_ok and calculated_scope == \
        footer.get("SCOPE_MANIFEST_SHA256") else "FAIL"
    if checks["Scope Hash"] == "FAIL":
        problems.append("Calculated SCOPE_MANIFEST_SHA256 与 Footer 不一致")
    sidecar_scope_ok = sidecar.get("scope_manifest_sha256") == \
        calculated_scope
    if not sidecar_scope_ok:
        problems.append("Sidecar scope_manifest_sha256 与 Calculated 不一致")

    # V50-V52 Calculated Root（用本次计算值，不用 Footer 声明值）→ Footer → Sidecar
    calculated_root = bundle_root_hash(calculated_scope,
                                       calculated_manifest)
    checks["Bundle Root"] = "PASS" if calculated_root == \
        footer.get("BUNDLE_ROOT_SHA256") else "FAIL"
    if checks["Bundle Root"] == "FAIL":
        problems.append("Calculated BUNDLE_ROOT_SHA256 与 Footer 不一致")
    sidecar_root_ok = sidecar.get("bundle_root_sha256") == calculated_root
    if not sidecar_root_ok:
        problems.append("Sidecar bundle_root_sha256 与 Calculated 不一致")

    # V60-V63 Sidecar 绑定
    sidecar_identity_problems = verify_sidecar_identity(
        sidecar, header, footer)
    problems.extend(sidecar_identity_problems)
    checks["Sidecar Identity"] = "PASS" if not sidecar_identity_problems \
        else "FAIL"

    bundle_records_payload = [asdict(r) for r in records]
    sidecar_records = sidecar.get("records")
    records_ok = (
        sidecar_records is not None
        and canonical_json(bundle_records_payload)
        == canonical_json(sidecar_records)
    )
    checks["Sidecar Records"] = "PASS" if records_ok else "FAIL"
    if not records_ok:
        problems.append("Sidecar records 与 Bundle records 不一致")

    sidecar_count = parse_int_field(sidecar, "merged_file_count", problems)
    sidecar_count_ok = (
        sidecar_count is not None
        and sidecar_count == actual_count
        and sidecar_count == footer_merged
    )
    checks["Sidecar File Count"] = "PASS" if sidecar_count_ok else "FAIL"
    if not sidecar_count_ok:
        problems.append("Sidecar merged_file_count 与 Bundle/Footer 不一致")

    # V63 Sidecar Error Counts：count 与 Footer 逐项一致（security 也一致，
    #    但 security 不要求为 0）
    sidecar_scan_errors = parse_int_field(
        sidecar, "scan_error_count", problems)
    sidecar_merge_errors = parse_int_field(
        sidecar, "merge_error_count", problems)
    sidecar_integrity_errors = parse_int_field(
        sidecar, "integrity_error_count", problems)
    sidecar_security_exclusions = parse_int_field(
        sidecar, "security_exclusion_count", problems)
    sidecar_error_counts_ok = (
        sidecar_scan_errors == footer_scan_errors
        and sidecar_merge_errors == footer_merge_errors
        and sidecar_integrity_errors == footer_integrity_errors
        and sidecar_security_exclusions == footer_security_exclusions
    )
    checks["Sidecar Error Counts"] = "PASS" if sidecar_error_counts_ok \
        else "FAIL"
    if not sidecar_error_counts_ok:
        problems.append("Sidecar error/exclusion counts 与 Footer 不一致")

    # V64 Sidecar Error Lists：list 长度 == 对应 count
    sidecar_error_lists_ok = all([
        sidecar_list_count_matches(sidecar, "scan_errors",
                                   sidecar_scan_errors),
        sidecar_list_count_matches(sidecar, "merge_errors",
                                   sidecar_merge_errors),
        sidecar_list_count_matches(sidecar, "integrity_errors",
                                   sidecar_integrity_errors),
        sidecar_list_count_matches(sidecar, "security_exclusions",
                                   sidecar_security_exclusions),
    ])
    checks["Sidecar Error Lists"] = "PASS" if sidecar_error_lists_ok \
        else "FAIL"
    if not sidecar_error_lists_ok:
        problems.append("Sidecar error/exclusion lists 与 count 不一致")

    # COMPLETE 时 Sidecar 也必须是 fail-closed：scan/merge/integrity 全 0 且空
    if footer.get("BUNDLE_STATUS") == "COMPLETE":
        sidecar_complete_errors_ok = (
            sidecar_scan_errors == 0
            and sidecar_merge_errors == 0
            and sidecar_integrity_errors == 0
            and sidecar.get("scan_errors") == []
            and sidecar.get("merge_errors") == []
            and sidecar.get("integrity_errors") == []
        )
        if not sidecar_complete_errors_ok:
            problems.append(
                "COMPLETE Bundle 的 Sidecar 仍存在 scan/merge/integrity 错误"
            )

    # V65 Sidecar Status
    checks["Sidecar Status"] = "PASS" if sidecar.get("bundle_status") == \
        footer.get("BUNDLE_STATUS") else "FAIL"
    # V66 Strict Mode：正式冻结不接受 strict_mode=false 的 evidence
    strict_mode_ok = sidecar.get("strict_mode") is True
    checks["Strict Mode"] = "PASS" if strict_mode_ok else "FAIL"
    if not strict_mode_ok:
        problems.append("Sidecar strict_mode 不是 true")
    checks["Sidecar Schema/Profile"] = "PASS" if (
        sidecar.get("bundle_schema") == BUNDLE_SCHEMA
        and sidecar.get("bundle_profile") == profile
    ) else "FAIL"
    if checks["Sidecar Status"] == "FAIL" \
            or checks["Sidecar Schema/Profile"] == "FAIL":
        problems.append("Sidecar status/schema/profile 与 Bundle 不一致")
    ...
    verification_ok = (
        not problems
        and all(value != "FAIL" for value in checks.values())
    )
    return {
        "verdict": "BUNDLE_VERIFIED" if verification_ok
