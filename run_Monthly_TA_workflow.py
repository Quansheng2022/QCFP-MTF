...
SCRIPT_LIST = [
    ...
    "Monthly_TA5_MergePDF.py",
]

# ========== 日志配置 ==========
# 控制程序日志：同时输出到文件和控制台
logger = logging.getLogger("monthlyWorkflow")
...
file_handler = logging.FileHandler(
    os.path.join(LOG_DIR, "run_monthly_workflow.log"),
    encoding="utf-8",
    mode="a"  # 追加模式，保留历史
)
...
def run_script(script_name: str, extra_args: list, date_str: str = None) -> bool:
    ...
    """
    script_path = os.path.join("Core","monthly", script_name)
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="TA_Workflow 月交易数据分析控制程序"
    )
    parser.add_argument(
        "--date",
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("TA_Workflow 月交易数据分析控制程序启动")
    ...
    logger.info("所有月交易数据分析脚本已按顺序成功执行完毕")
    logger.info("=" * 60)
