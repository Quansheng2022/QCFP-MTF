...
# 需要顺序执行的脚本列表（位于 Core/Quarterly 目录下）
SCRIPT_LIST = [
    ...
    "Quarterly_TA5_MergePDF.py",
]

# ========== 日志配置 ==========
# 控制程序日志：同时输出到文件和控制台
logger = logging.getLogger("quarterlyWorkflow")
...
file_handler = logging.FileHandler(
    os.path.join(LOG_DIR, "run_quarterly_workflow.log"),
    encoding="utf-8",
    mode="a"  # 追加模式，保留历史
)
...
def run_script(script_name: str, extra_args: list, date_str: str = None) -> bool:
    ...
    """
    script_path = os.path.join("Core", "Quarterly", script_name)
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="TA_Workflow 季交易数据分析控制程序"
    )
    parser.add_argument(
        "--date",
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("TA_Workflow 季交易数据分析控制程序启动")
    ...
    logger.info("所有季交易数据分析脚本已按顺序成功执行完毕")
    logger.info("=" * 60)
