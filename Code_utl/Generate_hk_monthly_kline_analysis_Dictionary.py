...
"""
Generate_hk_monthly_kline_analysis_Dictionary.py
生成 hk_monthly_kline_analysis 表的数据字典

功能：
1. 读取 SQLite 数据库中的 hk_monthly_kline_analysis 表结构
...
class Config:
    ...
    # 输出文件名
    JSON_FILENAME = "hk_monthly_kline_analysis_dictionary.json"
    MARKDOWN_FILENAME = "hk_monthly_kline_analysis_dictionary.md"
    EXCEL_FILENAME = "hk_monthly_kline_analysis_dictionary.xlsx"
...
class DataDictionaryGenerator:
    ...
    def _get_field_description(self, field_name: str) -> str:
        ...
        descriptions = {
            ...
            'ema5_10_streak': 'EMA5与EMA10连续状态天数（正:金叉持续, 负:死叉持续）',
            'ema5_20_status': 'EMA5与EMA20状态（1:金叉, -1:死叉, 0:粘合）',
            'ema5_20_streak': 'EMA5与EMA20连续状态天数（正:金叉持续, 负:死叉持续）',
            'ema10_50_status': 'EMA10与EMA50状态（1:金叉, -1:死叉, 0:粘合）',
            'ema10_50_streak': 'EMA10与EMA50连续状态天数（正:金叉持续, 负:死叉持续）',
            'ema20_50_status': 'EMA20与EMA50状态（1:金叉, -1:死叉, 0:粘合）',
            'ema20_50_streak': 'EMA20与EMA50连续状态天数（正:金叉持续, 负:死叉持续）',
...
def main():
    """主函数"""
    print("=" * 60)
    print("生成 hk_monthly_kline_analysis 数据字典")
    print("=" * 60)
    ...
    if not Config.DB_PATH.exists():
        ...
        print("2. 路径解析错误")
        print("\n解决方案：")
        print("1. 请先运行 Import_hk_monthly_kline_analysis.py 导入数据")
    ...
    try:
        ...
        with generator:
            ...
            if not generator.cursor.fetchone():
                ...
                print("请先运行 Import_hk_monthly_kline_analysis.py 创建表并导入数据")
                sys.exit(1)
