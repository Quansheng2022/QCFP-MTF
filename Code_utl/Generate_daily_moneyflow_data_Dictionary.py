...
"""
Generate_Moneyflow_Data_Dictionary.py
生成 hk_hist_daily_moneyflow 表的数据字典

功能：
1. 读取 SQLite 数据库中的 hk_hist_daily_moneyflow 表结构
...
class Config:
    ...
    # 输出文件名
    JSON_FILENAME = "hk_hist_daily_moneyflow_dictionary.json"
    MARKDOWN_FILENAME = "hk_hist_daily_moneyflow_dictionary.md"
    EXCEL_FILENAME = "hk_hist_daily_moneyflow_dictionary.xlsx"
...
class DataDictionaryGenerator:
    ...
    def get_field_statistics(self, table_name: str, field_name: str, field_type: str) -> Dict:
        ...
        try:
            ...
            # 如果是数值类型，计算统计值
            if field_type.upper() in ['REAL', 'FLOAT', 'DOUBLE', 'INTEGER', 'INT', 'NUMERIC']:
                try:
                    sql = f"""
                        SELECT 
    ...
    def _get_field_description(self, field_name: str) -> str:
        ...
        descriptions = {
            ...
            'Price_ChgPct': '价格涨跌幅（百分比）',
            'Capital_Trend': '资金趋势/净流入额',
            'Extra_Large': '特大单净流入额',
            'Large': '大单净流入额',
            'Medium': '中单净流入额',
            'Small': '小单净流入额',
            'capital_in_super': '特大单买入额',
            'capital_in_big': '大单买入额',
            'capital_in_mid': '中单买入额',
            'capital_in_small': '小单买入额',
            'capital_out_super': '特大单卖出额',
            'capital_out_big': '大单卖出额',
            'capital_out_mid': '中单卖出额',
            'capital_out_small': '小单卖出额'
        }
...
def main():
    """主函数"""
    print("=" * 60)
    print("生成 hk_hist_daily_moneyflow 数据字典")
    print("=" * 60)
    
    # 检查数据库是否存在
    if not Config.DB_PATH.exists():
        print(f"❌ 数据库文件不存在: {Config.DB_PATH}")
        print("请先运行 Daily_TA1A_download_moneyflow.py 下载数据")
        sys.exit(1)
