...
"""
Generate_hk_hist_monthly_kline_Dictionary.py
生成 hk_hist_monthly_kline 表的数据字典

功能：
1. 读取 SQLite 数据库中的 hk_hist_monthly_kline 表结构
...
class Config:
    ...
    # 输出文件名
    JSON_FILENAME = "hk_hist_monthly_kline_dictionary.json"
    MARKDOWN_FILENAME = "hk_hist_monthly_kline_dictionary.md"
    EXCEL_FILENAME = "hk_hist_monthly_kline_dictionary.xlsx"
...
class DataDictionaryGenerator:
    ...
    def _get_field_description(self, field_name: str) -> str:
        ...
        descriptions = {
            ...
            'date': '交易日期（格式：YYYY-MM-DD，每月最后一个交易日）',
            'open': '开盘价（元）',
            'high': '最高价（元）',
            'low': '最低价（元）',
            'close': '收盘价（元）',
...
def main():
    """主函数"""
    print("=" * 60)
    print("生成 hk_hist_monthly_kline 数据字典")
    print("=" * 60)
    ...
    if not Config.DB_PATH.exists():
        ...
        print("2. 路径解析错误")
        print("\n解决方案：")
        print("1. 请先运行 Import_HK_Hist_Monthly_KLine.py 导入数据")
    ...
    try:
        ...
        with generator:
            ...
            if not generator.cursor.fetchone():
                ...
                print("请先运行 Import_HK_Hist_Monthly_KLine.py 创建表并导入数据")
                sys.exit(1)
