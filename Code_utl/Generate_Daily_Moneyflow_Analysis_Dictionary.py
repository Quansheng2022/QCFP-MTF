...
"""
Generate_Daily_Moneyflow_Analysis_Dictionary.py
生成 hk_daily_moneyflow_analysis 表的数据字典

功能：
1. 读取 SQLite 数据库中的 hk_daily_moneyflow_analysis 表结构
...
class Config:
    ...
    # 输出文件名
    JSON_FILENAME = "hk_daily_moneyflow_analysis_dictionary.json"
    MARKDOWN_FILENAME = "hk_daily_moneyflow_analysis_dictionary.md"
    EXCEL_FILENAME = "hk_daily_moneyflow_analysis_dictionary.xlsx"
...
class DataDictionaryGenerator:
    ...
    def _get_field_description(self, field_name: str) -> str:
        ...
        descriptions = {
            ...
            'net_inflow_main_force': '主力资金净流入（万元）',
            'net_inflow_super_large': '超大单净流入（万元）',
            'net_inflow_large': '大单净流入（万元）',
            'net_inflow_medium': '中单净流入（万元）',
            'net_inflow_small': '小单净流入（万元）',
            'main_force_inflow_ratio': '主力资金流入占比（%）',
            'main_force_outflow_ratio': '主力资金流出占比（%）',
            'net_inflow_amount': '净流入总额（万元）',
            'turnover_rate': '换手率（%）',
            'volume_ratio': '量比',
            'main_force_activity': '主力活跃度指标',
            'money_flow_trend': '资金流向趋势（流入/流出/平衡）',
...
def main():
    """主函数"""
    print("=" * 60)
    print("生成 hk_daily_moneyflow_analysis 数据字典")
    print("=" * 60)
    ...
    if not Config.DB_PATH.exists():
        ...
        print("2. 路径解析错误")
        print("\n解决方案：")
        print("1. 请先运行 Import_Daily_Moneyflow_Analysis.py 导入数据")
    ...
    try:
        ...
        with generator:
            ...
            if not generator.cursor.fetchone():
                ...
                print("请先运行 Import_Daily_Moneyflow_Analysis.py 创建表并导入数据")
                sys.exit(1)
