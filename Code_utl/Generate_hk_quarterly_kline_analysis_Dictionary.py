...
"""
Generate_hk_quarterly_kline_analysis_Dictionary.py
生成 hk_quarterly_kline_analysis 表的数据字典

功能：
1. 读取 SQLite 数据库中的 hk_quarterly_kline_analysis 表结构
...
class Config:
    ...
    # 输出文件名
    JSON_FILENAME = "hk_quarterly_kline_analysis_dictionary.json"
    MARKDOWN_FILENAME = "hk_quarterly_kline_analysis_dictionary.md"
    EXCEL_FILENAME = "hk_quarterly_kline_analysis_dictionary.xlsx"
    
    # 表名
    TABLE_NAME = "hk_quarterly_kline_analysis"
...
class DataDictionaryGenerator:
    ...
    def _get_field_description(self, field_name: str) -> str:
        ...
        descriptions = {
            ...
            'ema100': '100期指数移动平均线',
            'ema200': '200期指数移动平均线',
            'macd_dif': 'MACD DIF线（差离值）',
            'macd_signal': 'MACD Signal线（信号线）',
            'macd_histogram': 'MACD Histogram（柱状线）',
            'ema5_10_status': 'EMA5与EMA10状态（1:金叉, -1:死叉, 0:粘合）',
            'ema5_10_streak': 'EMA5与EMA10连续状态季度数（正:金叉持续, 负:死叉持续）',
            'ema5_20_status': 'EMA5与EMA20状态（1:金叉, -1:死叉, 0:粘合）',
            'ema5_20_streak': 'EMA5与EMA20连续状态季度数（正:金叉持续, 负:死叉持续）',
            'ema10_50_status': 'EMA10与EMA50状态（1:金叉, -1:死叉, 0:粘合）',
            'ema10_50_streak': 'EMA10与EMA50连续状态季度数（正:金叉持续, 负:死叉持续）',
            'ema20_50_status': 'EMA20与EMA50状态（1:金叉, -1:死叉, 0:粘合）',
            'ema20_50_streak': 'EMA20与EMA50连续状态季度数（正:金叉持续, 负:死叉持续）',
            'golden_triangle': '黄金三角形态（1:形成, 0:未形成）',
            'death_triangle': '死亡三角形态（1:形成, 0:未形成）',
            'macd_cross': 'MACD交叉状态（1:金叉, -1:死叉, 0:无交叉）',
            'macd_status': 'MACD状态（1:DIF>Signal, -1:DIF<Signal）',
            'macd_golden_streak': 'MACD金叉持续天数',
            'macd_death_streak': 'MACD死叉持续天数',
            'vol_ema5': '成交量5期指数移动平均线',
            'volume_ratio5': '成交量与5期均量比',
            'gain': '当日上涨幅度（用于RSI计算）',
            'loss': '当日下跌幅度（用于RSI计算）',
            'avg_gain': '平均上涨幅度（14期）',
            'avg_loss': '平均下跌幅度（14期）',
    ...
    def save_markdown(self, dictionary: Dict, output_path: Path):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write("## 字段详细统计\n\n")
            for field in dictionary['fields']:
                if field['statistics'].get('min') is not None or field['statistics'].get('distinct_count'):
                    f.write(f"### {field['field_name']}\n\n")
                    ...
                    if 'distinct_count' in stats_info:
            ...
            if 'sample_data' in dictionary and dictionary['sample_data']:
                f.write("## 示例数据\n\n")
                # 只显示部分重要字段
                sample_keys = list(dictionary['sample_data'][0].keys())
                # 限制显示的字段数量，避免表格过宽
                display_keys = ['stock_code', 'stock_name', 'date', 'close', 'volume', 'rsi14', 'kdj_k', 'kdj_d', 'kdj_j']
                # 只显示存在的字段
                display_keys = [k for k in display_keys if k in sample_keys]
                
                f.write("| " + " | ".join(display_keys) + " |\n")
                f.write("|" + "|".join(["--------"] * len(display_keys)) + "|\n")
                
                for sample in dictionary['sample_data']:
                    values = [str(sample.get(k, '')) for k in display_keys]
                    f.write("| " + " | ".join(values) + " |\n")
...
def main():
    """主函数"""
    print("=" * 60)
    print("生成 hk_quarterly_kline_analysis 数据字典")
    print("=" * 60)
    ...
    if not Config.DB_PATH.exists():
        ...
        print("2. 路径解析错误")
        print("\n解决方案：")
        print("1. 请先运行 Import_hk_quarterly_kline_analysis.py 导入数据")
    ...
    try:
        ...
        with generator:
            ...
            if not generator.cursor.fetchone():
                ...
                print("请先运行 Import_hk_quarterly_kline_analysis.py 创建表并导入数据")
                sys.exit(1)
