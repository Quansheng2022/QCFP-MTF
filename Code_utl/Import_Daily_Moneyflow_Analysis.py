...
将日资金流分析数据CSV文件导入SQLite数据库
从CSV文件解析股票代码，匹配股票名称，创建hk_daily_moneyflow_analysis表

脚本位置: TA_Workflow2/Code_utl/Import_Daily_Moneyflow_Analysis.py
"""
...
def create_table_if_not_exists(conn, sample_file):
    """
    创建 hk_daily_moneyflow_analysis 表（如果不存在）
    ...
    try:
        ...
        # 添加其他列（排除stock_code和stock_name，因为我们已经添加了）
        for col in df_sample.columns:
            col_clean = clean_column_name(col)
            
            # 如果是stock_code或stock_name，跳过（我们已经添加了）
            if col_clean.lower() in ['stock_code', 'stock_name']:
                continue
            
            # 转换为小写
            col_clean_lower = col_clean.lower()
            
            # 根据列名和数据类型选择合适的SQLite类型
            # 资金流相关的列通常是REAL类型
            col_lower = col.lower()
            if any(keyword in col_lower for keyword in ['amount', 'volume', 'value', 'flow', 'money']):
                col_type = 'REAL'
            elif 'pct' in col_lower or 'change' in col_lower or 'rate' in col_lower:
                col_type = 'REAL'
            ...
            columns.append(f'"{col_clean_lower}" {col_type}')
        
        # 创建表
        create_sql = f"""
        CREATE TABLE IF NOT EXISTS hk_daily_moneyflow_analysis (
        ...
        print(f"[INFO] 表 hk_daily_moneyflow_analysis 已创建或已存在")
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE type='index' AND name='idx_moneyflow_stock_code_date'
            """)
            if not cursor.fetchone():
                create_index_sql = """
                CREATE INDEX idx_moneyflow_stock_code_date 
                ON hk_daily_moneyflow_analysis (stock_code, date)
                """
                conn.execute(create_index_sql)
                print(f"[INFO] 已创建复合索引 idx_moneyflow_stock_code_date")
            else:
                print(f"[INFO] 复合索引 idx_moneyflow_stock_code_date 已存在")
        except Exception as e:
...
def recreate_table(conn, sample_file):
    """
    重新创建表（删除旧表并创建新表）
    """
    try:
        # 删除旧表
        if table_exists(conn, 'hk_daily_moneyflow_analysis'):
            conn.execute("DROP TABLE hk_daily_moneyflow_analysis")
            print("[INFO] 已删除旧表 hk_daily_moneyflow_analysis")
        
        # 创建新表
        if create_table_if_not_exists(conn, sample_file):
            print("[INFO] 已重新创建表 hk_daily_moneyflow_analysis")
            return True
        else:
            print("[ERROR] 重新创建表失败")
            return False
...
def import_csv_to_db(csv_path, conn, stock_code, stock_name):
    ...
    try:
        ...
        # 清理列名（将%替换为Pct）并转换为小写
        original_columns = df.columns.tolist()
        cleaned_columns = [clean_column_name(col).lower() for col in original_columns]
        ...
        # 添加其他列（排除date列和可能存在的stock_code/stock_name列）
        for col in df.columns:
            if col.lower() not in ['date', 'stock_code', 'stock_name']:
                df_new[col] = df[col]
        ...
        # 导入数据
        df_new.to_sql('hk_daily_moneyflow_analysis', conn, if_exists='append', index=False)
...
def create_or_update_data_dictionary(conn, sample_file):
    """
    创建或更新数据字典 hk_daily_moneyflow_analysis_dictionary.json
    如果文件已经存在，则添加新的数据字典
    """
    print("\n" + "-"*60)
    print("创建/更新数据字典...")
    
    dict_path = project_dir / 'Config' / 'hk_daily_moneyflow_analysis_dictionary.json'
    ...
    data_dict = {
        'metadata': {
            ...
            'description': '港股日资金流分析数据表',
            'created_date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'version': '1.0',
            'database': str(DB_PATH.name),
            'data_source': 'CSV files from Data directory',
            'data_file': sample_file.name,
            'stock_code': code,
            'stock_name': STOCK_MAP.get(code, {}).get('name', f'股票_{code}'),
        },
        'table_schema': {
            'primary_key': 'id',
            ...
            'indexes': [
                {
                    'name': 'idx_moneyflow_stock_code_date',
        ...
        'stock_info': {
            'code': code,
            'name': STOCK_MAP.get(code, {}).get('name', f'股票_{code}'),
            'sector': STOCK_MAP.get(code, {}).get('sector', 'N/A'),
            'market': STOCK_MAP.get(code, {}).get('market', 'HKEX'),
        },
        'data_statistics': {
            'total_records': 0,
            'date_range': {
        ...
        'field_definitions': [],
        'usage_examples': {
            'query_all': "SELECT * FROM hk_daily_moneyflow_analysis LIMIT 10;",
            'query_by_stock': "SELECT * FROM hk_daily_moneyflow_analysis WHERE stock_code = '01951';",
            'query_date_range': "SELECT * FROM hk_daily_moneyflow_analysis WHERE date BETWEEN '2024-01-01' AND '2024-12-31';",
            'query_with_index': "SELECT * FROM hk_daily_moneyflow_analysis WHERE stock_code = '01951' AND date = '2024-01-01';"
        },
        'data_dictionary': {
            'column_definitions': [],
            'relationships': [],
    ...
    for col in df.columns:
        ...
        if 'flow' in col_lower and 'inflow' in col_lower:
            description = '资金流入'
        elif 'flow' in col_lower and 'outflow' in col_lower:
            description = '资金流出'
        elif 'net' in col_lower and 'flow' in col_lower:
            description = '净资金流'
        elif 'big' in col_lower and 'flow' in col_lower:
            description = '大单资金流'
        elif 'medium' in col_lower and 'flow' in col_lower:
            description = '中单资金流'
        elif 'small' in col_lower and 'flow' in col_lower:
            description = '小单资金流'
        elif 'pct' in col_lower or 'change' in col_lower:
            description = f'变化百分比: {col}'
        ...
        # 如果是包含Pct的字段，标记为百分比
        if 'pct' in col_lower or 'change' in col_lower:
            field_type = 'REAL'
        
        # 计算空值数量
        null_count = df[col].isnull().sum() if col in df.columns else 0
        ...
        field_def = {
            ...
            'sample_value': str(df[col].iloc[0]) if len(df) > 0 and not pd.isna(df[col].iloc[0]) else '',
            'example': '',
            'is_key': False,
            'is_required': False
        }
    ...
    for field in base_field_defs + data_dict['data_dictionary']['column_definitions']:
    ...
    if table_exists(conn, 'hk_daily_moneyflow_analysis'):
        try:
            ...
            # 总记录数
            cursor.execute("SELECT COUNT(*) FROM hk_daily_moneyflow_analysis")
    ...
    data_dict['data_dictionary']['business_rules'] = [
        {
            'rule': '每个股票每天只有一条记录',
            'description': '数据按日期去重，同一股票在同一天只有一条资金流分析数据'
        },
        {
            'rule': '资金流分类',
            'description': '包含大单、中单、小单资金流的详细分析'
        },
        {
            'rule': '索引优化',
    ...
    data_dict['data_dictionary']['relationships'] = [
        {
            ...
            'description': '可通过股票代码和日期与日K线数据关联'
        },
        {
            'type': '外键关联',
            'table': 'hk_idx_hist',
    ...
    if existing_dict:
        # 保留metadata中的创建日期等信息
        if 'metadata' in existing_dict and 'created_date' in existing_dict['metadata']:
            data_dict['metadata']['created_date'] = existing_dict['metadata'].get('created_date', data_dict['metadata']['created_date'])
        
        # 如果存在历史统计信息，可以保留
        if 'history' in existing_dict:
            data_dict['history'] = existing_dict['history']
        
        # 添加更新记录
        if 'update_history' not in data_dict:
            data_dict['update_history'] = []
        data_dict['update_history'].append({
            'update_date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'action': 'Updated',
            'stock_code': code,
            'stock_name': data_dict['stock_info']['name']
        })
    ...
    try:
        ...
        print(f"[INFO] 数据字典已保存: {dict_path}")
        print(f"[INFO] 数据字典大小: {len(json.dumps(data_dict))} 字节")
        
        # 显示数据字典摘要
        print("\n数据字典摘要:")
        print(f"   - 表名: {data_dict['metadata']['table_name']}")
        print(f"   - 股票: {data_dict['stock_info']['code']} {data_dict['stock_info']['name']}")
        ...
        if 'update_history' in data_dict:
            print(f"   - 更新记录数: {len(data_dict['update_history'])}")
...
def main():
    print("\n" + "="*60)
    print("导入日资金流分析数据到 SQLite 数据库")
    print("="*60 + "\n")
    
    # 查找CSV文件
    csv_pattern = "*_daily_moneyflow_analysis.csv"
    csv_files = list(DATA_DIR.glob(csv_pattern))
    
    if not csv_files:
        print(f"[ERROR] 在 {DATA_DIR} 中未找到 {csv_pattern} 文件")
    ...
    try:
        ...
        # 检查表是否存在
        if table_exists(conn, 'hk_daily_moneyflow_analysis'):
            existing_count = conn.execute("SELECT COUNT(*) FROM hk_daily_moneyflow_analysis").fetchone()[0]
            if existing_count > 0:
                print(f"\n[WARN] 表 hk_daily_moneyflow_analysis 中已有 {existing_count} 条记录")
                print("选项:")
                ...
                if response == 'd':
                    if not recreate_table(conn, sample_file):
                        print("[ERROR] 重新创建表失败")
                        return
                elif response == 'y' or response == 'yes':
                    clear_table(conn, 'hk_daily_moneyflow_analysis')
                    print("[INFO] 已清空旧数据")
                else:
                ...
                cursor = conn.cursor()
                cursor.execute("PRAGMA table_info(hk_daily_moneyflow_analysis)")
            ...
            # 表不存在，创建新表
            print("[INFO] 创建新表...")
            if not create_table_if_not_exists(conn, sample_file):
                print("[ERROR] 创建表失败")
                return
        ...
        # 显示表信息
        if table_exists(conn, 'hk_daily_moneyflow_analysis'):
            count = conn.execute("SELECT COUNT(*) FROM hk_daily_moneyflow_analysis").fetchone()[0]
            print(f"\n📋 表 hk_daily_moneyflow_analysis 当前共有 {count} 条记录")
            ...
            # 显示索引
            cursor.execute("SELECT name, sql FROM sqlite_master WHERE type='index' AND tbl_name='hk_daily_moneyflow_analysis'")
            ...
            # 显示股票统计
            stocks = conn.execute(
                "SELECT DISTINCT stock_code, stock_name FROM hk_daily_moneyflow_analysis ORDER BY stock_code"
            ).fetchall()
            print(f"\n📈 包含 {len(stocks)} 只股票的数据:")
            for code, name in stocks:
                cnt = conn.execute(
                    f"SELECT COUNT(*) FROM hk_daily_moneyflow_analysis WHERE stock_code='{code}'"
                ).fetchone()[0]
                in_config = "✓" if code in STOCK_MAP else "⚠️"
                print(f"   {in_config} {code} {name}: {cnt} 条记录")
        
        # 创建/更新数据字典
        create_or_update_data_dictionary(conn, sample_file)
