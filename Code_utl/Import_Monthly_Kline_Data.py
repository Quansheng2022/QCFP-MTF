...
将月交易数据CSV文件导入SQLite数据库
从CSV文件解析股票代码，匹配股票名称，创建hk_hist_monthly_kline表

脚本位置: TA_Workflow2/Code_utl/Import_Monthly_Kline_Data.py
"""
...
def create_table_if_not_exists(conn):
    """
    创建 hk_hist_monthly_kline 表（如果不存在）
    将 stock_code 和 stock_name 放在最前面
    创建复合索引 (stock_code, date)
    """
    # 先读取一个CSV文件样例来获取列结构
    csv_files = list(DATA_DIR.glob("*_monthly_historical_data.csv"))
    if not csv_files:
        print(f"[ERROR] 未找到任何 *_monthly_historical_data.csv 文件")
        return False
    ...
    try:
        ...
        for col in df_sample.columns:
            if '%' in col:
                new_col = col.replace('%', 'Pct')
                cleaned_columns[col] = new_col
                print(f"[INFO] 列名转换: {col} -> {new_col}")
            else:
                cleaned_columns[col] = col
        
        # 重命名列
        df_sample = df_sample.rename(columns=cleaned_columns)
        ...
        # 添加其他列（排除可能的date列，因为我们已经添加了date字段）
        for col in df_sample.columns:
            col_clean = col.strip().replace(' ', '_').replace('/', '_').replace('(', '').replace(')', '').replace('%', 'Pct')
            ...
            if col.lower() in ['open', 'high', 'low', 'close', 'adj_close', 'adjclose']:
                col_type = 'REAL'
            elif 'volume' in col.lower() or 'amount' in col.lower() or 'turnover' in col.lower():
                col_type = 'REAL'
            elif 'amplitude' in col.lower() or 'change' in col.lower() or 'pct' in col.lower():
                col_type = 'REAL'
            else:
                # 检查数据类型
                try:
        ...
        print(f"[INFO] 表 hk_hist_monthly_kline 已创建或已存在")
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE type='index' AND name='idx_monthly_stock_code_date'
            """)
            if not cursor.fetchone():
                create_index_sql = """
                CREATE INDEX idx_monthly_stock_code_date 
                ON hk_hist_monthly_kline (stock_code, date)
                """
                conn.execute(create_index_sql)
                print(f"[INFO] 已创建复合索引 idx_monthly_stock_code_date")
            else:
                print(f"[INFO] 复合索引 idx_monthly_stock_code_date 已存在")
        except Exception as e:
...
def recreate_table(conn):
    ...
    try:
        # 删除旧表
        if table_exists(conn, 'hk_hist_monthly_kline'):
            conn.execute("DROP TABLE hk_hist_monthly_kline")
            print("[INFO] 已删除旧表 hk_hist_monthly_kline")
        
        # 创建新表
        if create_table_if_not_exists(conn):
            print("[INFO] 已重新创建表 hk_hist_monthly_kline")
            return True
        else:
            print("[ERROR] 重新创建表失败")
            return False
    ...
    try:
        ...
        for col in df.columns:
            if '%' in col:
                ...
                cleaned_columns[col] = col.strip().replace(' ', '_').replace('/', '_').replace('(', '').replace(')', '')
        ...
        # 导入数据
        df_new.to_sql('hk_hist_monthly_kline', conn, if_exists='append', index=False)
...
def create_data_dictionary(conn):
    """
    创建完整的数据字典 hk_hist_monthly_kline.json
    ...
    if dict_path.exists():
        try:
            ...
            print(f"[INFO] 数据字典文件已存在，将更新内容")
        except Exception as e:
            print(f"[WARN] 读取现有数据字典失败: {e}，将创建新文件")
    ...
    data_dict = {
        ...
        'table_schema': {
            'primary_key': 'id',
            'columns_order': ['stock_code', 'stock_name', 'date', 'Open', 'High', 'Low', 'Close', 'Volume', '...'],
            'columns': [],
            'indexes': [
                {
                    'name': 'idx_monthly_stock_code_date',
                    'fields': ['stock_code', 'date'],
                    'type': '复合索引',
                    'description': '用于快速查询特定股票的月交易数据'
                }
            ],
            'constraints': [
                'PRIMARY KEY (id)',
        ...
        'field_definitions': [],
        'usage_examples': {
            'query_all': "SELECT * FROM hk_hist_monthly_kline LIMIT 10;",
            'query_by_stock': "SELECT * FROM hk_hist_monthly_kline WHERE stock_code = '01951';",
            'query_date_range': "SELECT * FROM hk_hist_monthly_kline WHERE date BETWEEN '2024-01-01' AND '2024-12-31';",
            'query_closing_prices': "SELECT date, stock_code, stock_name, close FROM hk_hist_monthly_kline;",
            'query_with_index': "SELECT * FROM hk_hist_monthly_kline WHERE stock_code = '01951' AND date = '2024-01-01';"
        },
        'data_dictionary': {
            'column_definitions': [],
            'relationships': [],
    ...
    if sample_df is not None:
        for col in sample_df.columns:
            ...
            if 'open' in col_lower:
                ...
                constraint = ''
                example = '3.5'
            elif 'change' in col_lower and 'pct' in col_lower:
                field_type = 'REAL'
                description = '月涨跌幅（%）'
                constraint = ''
                example = '2.5'
                ...
                constraint = ''
                example = '1.5'
            elif 'previous' in col_lower and 'close' in col_lower:
                field_type = 'REAL'
                description = '前月收盘价'
                constraint = ''
                example = '10.55'
    ...
    data_dict['data_dictionary']['business_rules'] = [
        {
            'rule': '每个股票每月只有一条记录',
            'description': '数据按月去重，同一股票在同一个月只有一条交易数据'
        },
        {
            'rule': '价格精度',
        ...
        {
            'rule': '数据完整性',
            'description': '所有交易数据应包含完整的OHLC（开高低收）和成交量信息'
        },
        {
            'rule': '索引优化',
    ...
    data_dict['data_dictionary']['relationships'] = [
        ...
        },
        {
            ...
            'description': '可通过股票代码和日期与日线数据关联，进行周期转换分析'
        }
    ]
    
    # 如果数据字典已存在，更新而不是覆盖
    if dict_path.exists():
        # 保留原有的某些信息（如历史记录），但更新当前数据
        if 'metadata' in existing_dict:
            if 'created_date' in existing_dict['metadata']:
                data_dict['metadata']['created_date'] = existing_dict['metadata']['created_date']
            if 'version_history' in existing_dict['metadata']:
                data_dict['metadata']['version_history'] = existing_dict['metadata']['version_history']
        
        # 添加更新历史
        if 'metadata' not in data_dict:
            data_dict['metadata'] = {}
        data_dict['metadata']['last_updated'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        data_dict['metadata']['update_count'] = existing_dict.get('metadata', {}).get('update_count', 0) + 1
        
        print(f"[INFO] 更新现有数据字典，更新次数: {data_dict['metadata']['update_count']}")
...
def main():
    ...
    if not csv_files:
        print(f"[ERROR] 在 {DATA_DIR} 中未找到 *_monthly_historical_data.csv 文件")
    ...
    try:
        # 检查表是否存在
        if table_exists(conn, 'hk_hist_monthly_kline'):
            existing_count = conn.execute("SELECT COUNT(*) FROM hk_hist_monthly_kline").fetchone()[0]
            if existing_count > 0:
                print(f"\n[WARN] 表 hk_hist_monthly_kline 中已有 {existing_count} 条记录")
                print("选项:")
                ...
                if response == 'd':
                    ...
                    clear_table(conn, 'hk_hist_monthly_kline')
                    print("[INFO] 已清空旧数据")
                else:
                ...
                # 获取表结构
                cursor = conn.cursor()
                cursor.execute("PRAGMA table_info(hk_hist_monthly_kline)")
        ...
        # 显示表信息
        if table_exists(conn, 'hk_hist_monthly_kline'):
            count = conn.execute("SELECT COUNT(*) FROM hk_hist_monthly_kline").fetchone()[0]
            print(f"\n📋 表 hk_hist_monthly_kline 当前共有 {count} 条记录")
            ...
            # 显示索引
            cursor.execute("SELECT name, sql FROM sqlite_master WHERE type='index' AND tbl_name='hk_hist_monthly_kline'")
            ...
            # 显示股票统计
            stocks = conn.execute(
                "SELECT DISTINCT stock_code, stock_name FROM hk_hist_monthly_kline ORDER BY stock_code"
            ).fetchall()
            print(f"\n📈 包含 {len(stocks)} 只股票的数据:")
            for code, name in stocks:
                cnt = conn.execute(
                    f"SELECT COUNT(*) FROM hk_hist_monthly_kline WHERE stock_code='{code}'"
                ).fetchone()[0]
                # 检查是否在配置文件中
