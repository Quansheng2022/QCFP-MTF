...
"""
Generate_hk_hist_daily_kline_Dictionary.py
生成 hk_hist_daily_kline 表的数据字典

功能：
1. 读取 SQLite 数据库中的 hk_hist_daily_kline 表结构
...
class Config:
    ...
    # 输出文件名
    JSON_FILENAME = "hk_hist_daily_kline_dictionary.json"
    MARKDOWN_FILENAME = "hk_hist_daily_kline_dictionary.md"
    EXCEL_FILENAME = "hk_hist_daily_kline_dictionary.xlsx"
...
class DataDictionaryGenerator:
    ...
    def get_table_statistics(self, table_name: str) -> Dict:
        ...
        try:
            ...
            # 价格范围
            self.cursor.execute(f"SELECT MIN(close), MAX(close) FROM {table_name}")
            row = self.cursor.fetchone()
            stats['min_close'] = row[0] if row[0] else None
            stats['max_close'] = row[1] if row[1] else None
            
            # 成交量范围
            self.cursor.execute(f"SELECT MIN(volume), MAX(volume) FROM {table_name}")
            row = self.cursor.fetchone()
            stats['min_volume'] = row[0] if row[0] else None
            stats['max_volume'] = row[1] if row[1] else None
    ...
    def save_markdown(self, dictionary: Dict, output_path: Path):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            if stats and 'error' not in stats:
                ...
                if stats.get('min_close') is not None:
                    f.write(f"- **收盘价范围**: {stats['min_close']:.2f} 至 {stats['max_close']:.2f}\n")
                if stats.get('min_volume') is not None:
                    f.write(f"- **成交量范围**: {stats['min_volume']:,.0f} 至 {stats['max_volume']:,.0f}\n")
                f.write("\n")
            ...
            for field in dictionary['fields']:
                ...
                if stats_info.get('min') is not None or stats_info.get('distinct_count') is not None:
                    ...
                    elif 'distinct_count' in stats_info:
            ...
            if 'sample_data' in dictionary and dictionary['sample_data']:
                f.write("## 示例数据\n\n")
                # 获取所有列名
                columns = list(dictionary['sample_data'][0].keys())
        ...
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            ...
            if stats and 'error' not in stats:
                ...
                if 'total_records' in stats:
                    stats_items.append('总记录数')
                    stats_values.append(stats.get('total_records', 0))
                if 'stock_count' in stats:
                    stats_items.append('股票数量')
                    stats_values.append(stats.get('stock_count', 0))
                if 'min_date' in stats:
                    stats_items.append('最早日期')
                    stats_values.append(stats.get('min_date', ''))
                if 'max_date' in stats:
                    stats_items.append('最晚日期')
                    stats_values.append(stats.get('max_date', ''))
                if 'min_close' in stats:
                    stats_items.append('最低收盘价')
                    stats_values.append(stats.get('min_close', ''))
                if 'max_close' in stats:
                    stats_items.append('最高收盘价')
                    stats_values.append(stats.get('max_close', ''))
                if 'min_volume' in stats:
                    stats_items.append('最小成交量')
                    stats_values.append(stats.get('min_volume', ''))
                if 'max_volume' in stats:
                    stats_items.append('最大成交量')
                    stats_values.append(stats.get('max_volume', ''))
...
def main():
    """主函数"""
    print("=" * 60)
    print("生成 hk_hist_daily_kline 数据字典")
    print("=" * 60)
    ...
    if not Config.DB_PATH.exists():
        ...
        print("2. 路径解析错误")
        print("\n解决方案：")
        print("1. 请先运行 Import_HK_Hist_Daily_Kline.py 导入数据")
    ...
    try:
        ...
        with generator:
            ...
            if not generator.cursor.fetchone():
                ...
                print("请先运行 Import_HK_Hist_Daily_Kline.py 创建表并导入数据")
                sys.exit(1)
            ...
            if dictionary['statistics'].get('min_close') is not None:
                print(f"   - 收盘价范围: {dictionary['statistics']['min_close']:.2f} 至 {dictionary['statistics']['max_close']:.2f}")
