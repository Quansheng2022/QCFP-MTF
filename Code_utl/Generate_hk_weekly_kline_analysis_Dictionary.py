...
"""
Generate_hk_weekly_kline_analysis_Dictionary.py
生成 hk_weekly_kline_analysis 表的数据字典

功能：
1. 读取 SQLite 数据库中的 hk_weekly_kline_analysis 表结构
...
class Config:
    ...
    # 输出文件名
    JSON_FILENAME = "hk_weekly_kline_analysis_dictionary.json"
    MARKDOWN_FILENAME = "hk_weekly_kline_analysis_dictionary.md"
    EXCEL_FILENAME = "hk_weekly_kline_analysis_dictionary.xlsx"
...
class DataDictionaryGenerator:
    ...
    def _get_field_description(self, field_name: str) -> str:
        ...
        descriptions = {
            ...
            'date': '周K线日期（格式：YYYY-MM-DD，每周最后一个交易日）',
            'open': '周开盘价',
            'high': '周最高价',
            'low': '周最低价',
            'close': '周收盘价',
            ...
            'macd_histogram': 'MACD 柱状图（DIF - Signal）',
            'ema5_10_status': 'EMA5与EMA10交叉状态（金叉/死叉/平行）',
            'ema5_10_streak': 'EMA5与EMA10持续状态天数',
            'ema5_20_status': 'EMA5与EMA20交叉状态（金叉/死叉/平行）',
            'ema5_20_streak': 'EMA5与EMA20持续状态天数',
            'ema10_50_status': 'EMA10与EMA50交叉状态（金叉/死叉/平行）',
            'ema10_50_streak': 'EMA10与EMA50持续状态天数',
            'ema20_50_status': 'EMA20与EMA50交叉状态（金叉/死叉/平行）',
            'ema20_50_streak': 'EMA20与EMA50持续状态天数',
            'golden_triangle': '黄金三角形（1: 金叉形成, 0: 未形成）',
            'death_triangle': '死亡三角形（1: 死叉形成, 0: 未形成）',
            'macd_cross': 'MACD交叉信号（1: 金叉, -1: 死叉, 0: 无交叉）',
            'macd_status': 'MACD状态（1: 多头, -1: 空头, 0: 震荡）',
            'macd_golden_streak': 'MACD金叉持续周数',
            'macd_death_streak': 'MACD死叉持续周数',
            'kdj_k': 'KDJ指标K值',
            'kdj_d': 'KDJ指标D值',
            'kdj_j': 'KDJ指标J值',
            'vol_ema5': '5周成交量指数移动平均线',
            'volume_ratio': '量比（成交量/5周均量）',
            'gain': '本周涨幅（正数）',
            'loss': '本周跌幅（负数）',
            'avg_gain': '14周平均涨幅',
            'avg_loss': '14周平均跌幅',
...
def main():
    """主函数"""
    print("=" * 60)
    print("生成 hk_weekly_kline_analysis 数据字典")
    print("=" * 60)
    ...
    if not Config.DB_PATH.exists():
        ...
        print("2. 路径解析错误")
        print("\n解决方案：")
        print("1. 请先运行 Import_HK_Weekly_KLine_Analysis.py 导入数据")
    ...
    try:
        ...
        with generator:
            ...
            if not generator.cursor.fetchone():
                ...
                print("请先运行 Import_HK_Weekly_KLine_Analysis.py 创建表并导入数据")
                sys.exit(1)
