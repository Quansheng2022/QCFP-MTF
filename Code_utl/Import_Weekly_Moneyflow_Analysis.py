...
将周资金流分析数据CSV文件导入SQLite数据库
从CSV文件解析股票代码，匹配股票名称，创建hk_weekly_moneyflow_analysis表

脚本位置: TA_Workflow2/Code_utl/Import_Weekly_Moneyflow_Analysis.py
"""
...
def sanitize_column_name(col_name):
    ...
    # 去除其他特殊字符
    col_name = col_name.replace('(', '').replace(')', '').replace(' ', '_').replace('/', '_')
...
def create_table_if_not_exists(conn):
    """
    创建 hk_weekly_moneyflow_analysis 表（如果不存在）
    将 stock_code 和 stock_name 放在最前面
    创建复合索引 (stock_code, date)
    所有列名转换为小写
    """
    # 先读取一个CSV文件样例来获取列结构
    csv_files = list(DATA_DIR.glob("*_weekly_moneyflow_analysis.csv"))
    if not csv_files:
        print(f"[ERROR] 未找到任何 *_weekly_moneyflow_analysis.csv 文件")
        return False
    ...
    try:
        ...
        for col in df_sample.columns:
            ...
            if any(keyword in col_clean for keyword in ['close', 'price', 'amount', 'value']):
                col_type = 'real'
            elif any(keyword in col_clean for keyword in ['volume', 'shares', 'turnover']):
                col_type = 'real'
            elif any(keyword in col_clean for keyword in ['pct', 'change', 'ratio', 'rate']):
                col_type = 'real'
            elif any(keyword in col_clean for keyword in ['count', 'num', 'number']):
                col_type = 'integer'
            else:
                # 检查数据类型
                try:
        ...
        # 创建表
        create_sql = f"""
        CREATE TABLE IF NOT EXISTS hk_weekly_moneyflow_analysis (
        ...
        print(f"[INFO] 表 hk_weekly_moneyflow_analysis 已创建或已存在")
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE type='index' AND name='idx_weekly_moneyflow_code_date'
            """)
            if not cursor.fetchone():
                create_index_sql = """
                CREATE INDEX idx_weekly_moneyflow_code_date 
                ON hk_weekly_moneyflow_analysis (stock_code, date)
                """
                conn.execute(create_index_sql)
                print(f"[INFO] 已创建复合索引 idx_weekly_moneyflow_code_date")
            else:
                print(f"[INFO] 复合索引 idx_weekly_moneyflow_code_date 已存在")
        except Exception as e:
...
def recreate_table(conn):
    ...
    try:
        # 删除旧表
        if table_exists(conn, 'hk_weekly_moneyflow_analysis'):
            conn.execute("DROP TABLE hk_weekly_moneyflow_analysis")
            print("[INFO] 已删除旧表 hk_weekly_moneyflow_analysis")
        
        # 创建新表
        if create_table_if_not_exists(conn):
            print("[INFO] 已重新创建表 hk_weekly_moneyflow_analysis")
            return True
        else:
            print("[ERROR] 重新创建表失败")
            return False
...
def import_csv_to_db(csv_path, conn, stock_code, stock_name):
    ...
    try:
        ...
        # 导入数据
        df_new.to_sql('hk_weekly_moneyflow_analysis', conn, if_exists='append', index=False)
...
def create_data_dictionary(conn):
    """
    创建完整的数据字典 hk_weekly_moneyflow_analysis_dictionary.json
    如果数据字典文件已经存在，则添加新的数据字典
    如果表已经存在，则先删除，创建该表的数据字典
    """
    print("\n" + "-"*60)
    print("创建数据字典...")
    ...
    existing_dict = None
    if dict_path.exists():
        print(f"[INFO] 数据字典文件已存在: {dict_path}")
        try:
            with open(dict_path, 'r', encoding='utf-8') as f:
                existing_dict = json.load(f)
            print(f"[INFO] 加载现有数据字典成功")
        except Exception as e:
    ...
    for csv_file in csv_files:
        try:
            ...
            if sample_df is None:
                sample_df = df
                # 清理示例DataFrame的列名并转换为小写
                sample_df.columns = [sanitize_column_name(col) for col in sample_df.columns]
        except Exception as e:
    ...
    data_dict = {
        ...
        'table_schema': {
            ...
            'indexes': [
                {
                    'name': 'idx_weekly_moneyflow_code_date',
                    'fields': ['stock_code', 'date'],
                    'type': '复合索引',
                    'description': '用于快速查询特定股票的周资金流数据'
                }
            ],
            'constraints': [
                'PRIMARY KEY (id)',
        ...
        'field_definitions': [],
        'usage_examples': {
            'query_all': "SELECT * FROM hk_weekly_moneyflow_analysis LIMIT 10;",
            'query_by_stock': "SELECT * FROM hk_weekly_moneyflow_analysis WHERE stock_code = '01951';",
            'query_date_range': "SELECT * FROM hk_weekly_moneyflow_analysis WHERE date BETWEEN '2024-01-01' AND '2024-12-31';",
            'query_moneyflow': "SELECT date, stock_code, stock_name, net_inflow FROM hk_weekly_moneyflow_analysis;",
            'query_with_index': "SELECT * FROM hk_weekly_moneyflow_analysis WHERE stock_code = '01951' AND date = '2024-01-01';"
        },
        'data_dictionary': {
            'column_definitions': [],
            'relationships': [],
    ...
    base_fields = [
        ...
        {'name': 'date', 'type': 'text', 'description': '交易日期（YYYY-MM-DD格式，每周最后交易日）', 'constraint': 'NOT NULL', 'example': '2024-01-05', 'is_key': True, 'is_required': True},
    ]
    ...
    if sample_df is not None:
        for col in sample_df.columns:
            ...
            # 资金流相关字段
            if 'net' in col_lower and ('inflow' in col_lower or 'outflow' in col_lower):
                field_type = 'real'
                description = '净资金流（万元）' if 'net' in col_lower else '资金流'
                constraint = ''
                example = '1234.56'
            elif 'main' in col_lower and ('inflow' in col_lower or 'outflow' in col_lower):
                field_type = 'real'
                description = '主力资金流（万元）'
                constraint = ''
                example = '5678.90'
            elif 'retail' in col_lower and ('inflow' in col_lower or 'outflow' in col_lower):
                field_type = 'real'
                description = '散户资金流（万元）'
                constraint = ''
                example = '234.56'
            elif 'institutional' in col_lower and ('inflow' in col_lower or 'outflow' in col_lower):
                field_type = 'real'
                description = '机构资金流（万元）'
                constraint = ''
                example = '7890.12'
    ...
    if table_exists(conn, 'hk_weekly_moneyflow_analysis'):
        try:
            ...
            # 总记录数
            cursor.execute("SELECT COUNT(*) FROM hk_weekly_moneyflow_analysis")
    ...
    data_dict['data_dictionary']['business_rules'] = [
        {
            'rule': '每个股票每周只有一条记录',
            'description': '数据按周去重，同一股票在同一周只有一条资金流数据'
        },
        {
            'rule': '资金流单位',
            'description': '资金流数据单位为万元'
        },
        {
            'rule': '数据精度',
    ...
    data_dict['data_dictionary']['relationships'] = [
        {
            ...
            'description': '可通过股票代码和日期与日线数据关联'
        },
        {
            'type': '外键关联',
            'table': 'hk_weekly_moneyflow_analysis',
            'field': 'date',
            'description': '可通过日期字段与其他周数据关联'
        }
    ]
    ...
    if existing_dict:
        ...
        # 保留之前的统计信息作为历史
        if 'data_statistics' in existing_dict and 'stocks_loaded' in existing_dict['data_statistics']:
            if existing_dict['data_statistics']['stocks_loaded']:
                data_dict['data_statistics']['previous_stocks'] = existing_dict['data_statistics']['stocks_loaded']
        
        # 更新版本
        data_dict['metadata']['version'] = '1.1'
        data_dict['metadata']['updated_date'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        data_dict['metadata']['update_note'] = '合并现有数据字典，添加新数据'
...
def main():
    ...
    if not csv_files:
        print(f"[ERROR] 在 {DATA_DIR} 中未找到 *_weekly_moneyflow_analysis.csv 文件")
    ...
    try:
        # 检查表是否存在
        if table_exists(conn, 'hk_weekly_moneyflow_analysis'):
            existing_count = conn.execute("SELECT COUNT(*) FROM hk_weekly_moneyflow_analysis").fetchone()[0]
            if existing_count > 0:
                print(f"\n[WARN] 表 hk_weekly_moneyflow_analysis 中已有 {existing_count} 条记录")
                print("选项:")
                ...
                if response == 'd':
                    ...
                    clear_table(conn, 'hk_weekly_moneyflow_analysis')
                    print("[INFO] 已清空旧数据")
                else:
                ...
                # 获取表结构
                cursor = conn.cursor()
                cursor.execute("PRAGMA table_info(hk_weekly_moneyflow_analysis)")
        ...
        # 显示表信息
        if table_exists(conn, 'hk_weekly_moneyflow_analysis'):
            count = conn.execute("SELECT COUNT(*) FROM hk_weekly_moneyflow_analysis").fetchone()[0]
            print(f"\n📋 表 hk_weekly_moneyflow_analysis 当前共有 {count} 条记录")
            
            # 显示表结构
            cursor = conn.cursor()
            cursor.execute("PRAGMA table_info(hk_weekly_moneyflow_analysis)")
            columns = cursor.fetchall()
            print(f"📋 表结构（字段顺序，所有列名已转换为小写）:")
            for i, col in enumerate(columns, 1):
                print(f"   {i}. {col[1]} ({col[2]}) {'NOT NULL' if col[3] else ''}")
            
            # 显示索引
            cursor.execute("SELECT name, sql FROM sqlite_master WHERE type='index' AND tbl_name='hk_weekly_moneyflow_analysis'")
            ...
            # 显示股票统计
            stocks = conn.execute(
                "SELECT DISTINCT stock_code, stock_name FROM hk_weekly_moneyflow_analysis ORDER BY stock_code"
            ).fetchall()
            print(f"\n📈 包含 {len(stocks)} 只股票的数据:")
            for code, name in stocks:
                cnt = conn.execute(
                    f"SELECT COUNT(*) FROM hk_weekly_moneyflow_analysis WHERE stock_code='{code}'"
                ).fetchone()[0]
                # 检查是否在配置文件中
