...
将周资金流数据CSV文件导入SQLite数据库
从CSV文件解析股票代码，匹配股票名称，创建hk_hist_weekly_moneyflow表

脚本位置: TA_Workflow2/Code_utl/Import_Weekly_Moneyflow.py
"""
...
def create_table_if_not_exists(conn):
    """
    创建 hk_hist_weekly_moneyflow 表（如果不存在）
    将 stock_code 和 stock_name 放在最前面
    所有列名转换为小写
    创建复合索引 (stock_code, date)
    """
    # 先读取一个CSV文件样例来获取列结构
    csv_files = list(DATA_DIR.glob("*_weekly_moneyflow.csv"))
    if not csv_files:
        print(f"[ERROR] 未找到任何 *_weekly_moneyflow.csv 文件")
        return False
    ...
    try:
        ...
        for col in df_sample.columns:
            ...
            columns_mapping[col] = col_clean
        
        # 重命名列
        if columns_mapping:
            df_sample = df_sample.rename(columns=columns_mapping)
        ...
        for col in df_sample.columns:
            col_clean = col.strip().lower().replace(' ', '_').replace('/', '_').replace('(', '').replace(')', '')
            ...
            if 'price' in col_clean or 'amount' in col_clean or 'value' in col_clean:
                col_type = 'REAL'
            elif 'volume' in col_clean or 'shares' in col_clean:
                col_type = 'REAL'
            elif 'pct' in col_clean or 'rate' in col_clean:
                col_type = 'REAL'
            elif 'flow' in col_clean:
                col_type = 'REAL'
            else:
                # 检查数据类型
                try:
        ...
        print(f"[INFO] 表 hk_hist_weekly_moneyflow 已创建或已存在（所有列名已转为小写）")
        ...
        try:
            ...
            if not cursor.fetchone():
                ...
                ON hk_hist_weekly_moneyflow (stock_code, date)
                """
...
def recreate_table(conn):
    ...
    try:
        # 删除旧表
        if table_exists(conn, 'hk_hist_weekly_moneyflow'):
            conn.execute("DROP TABLE hk_hist_weekly_moneyflow")
            print("[INFO] 已删除旧表 hk_hist_weekly_moneyflow")
        
        # 创建新表
        if create_table_if_not_exists(conn):
            print("[INFO] 已重新创建表 hk_hist_weekly_moneyflow（所有列名已转为小写）")
            return True
        else:
            print("[ERROR] 重新创建表失败")
            return False
...
def import_csv_to_db(csv_path, conn, stock_code, stock_name):
    ...
    确保字段顺序与表结构匹配
    所有列名转换为小写
    处理Price_Chg%列：重命名为price_chgpct，数值x100
    """
    try:
        # 读取CSV文件
        df = pd.read_csv(csv_path)
        
        # 处理列名：将含有%的列名改成...Pct，全部转换为小写
        columns_mapping = {}
        ...
        if columns_mapping:
            ...
            print(f"   [INFO] 重命名列（转为小写）: {columns_mapping}")
        
        # 清理列名
        df.columns = [col.strip().lower().replace(' ', '_').replace('/', '_').replace('(', '').replace(')', '') for col in df.columns]
        
        # 处理 price_chgpct 列：数值 x100
        price_chg_col = None
        for col in df.columns:
            if 'price_chgpct' in col or 'price_chg_pct' in col:
                price_chg_col = col
                break
        
        if price_chg_col and price_chg_col in df.columns:
            try:
                # 将数值 x100
                df[price_chg_col] = pd.to_numeric(df[price_chg_col], errors='coerce') * 100
                print(f"   [INFO] price_chgpct 列数值已乘以100")
            except Exception as e:
                print(f"   [WARN] 处理 price_chgpct 列失败: {e}")
        ...
        # 导入数据
        df_new.to_sql('hk_hist_weekly_moneyflow', conn, if_exists='append', index=False)
...
def create_data_dictionary(conn):
    """
    创建完整的数据字典 hk_hist_weekly_moneyflow_dictionary.json
    ...
    dict_path = project_dir / 'Config' / 'hk_hist_weekly_moneyflow_dictionary.json'
    ...
    for csv_file in csv_files:
        try:
            ...
            file_info.append({
                ...
                'code': code,
                'rows': len(df),
                'columns': [col.lower() for col in df.columns],  # 转换为小写
                'date_range': {
            ...
            all_columns.update([col.lower() for col in df.columns])  # 转换为小写
    ...
    data_dict = {
        'metadata': {
            ...
            'description': '港股周资金流历史数据表（所有列名小写）',
        ...
        'field_definitions': [],
        'usage_examples': {
            'query_all': "SELECT * FROM hk_hist_weekly_moneyflow LIMIT 10;",
            'query_by_stock': "SELECT * FROM hk_hist_weekly_moneyflow WHERE stock_code = '01951';",
            'query_date_range': "SELECT * FROM hk_hist_weekly_moneyflow WHERE date BETWEEN '2024-01-01' AND '2024-12-31';",
            'query_moneyflow': "SELECT date, stock_code, stock_name, main_flow, retail_flow FROM hk_hist_weekly_moneyflow;",
            'query_with_index': "SELECT * FROM hk_hist_weekly_moneyflow WHERE stock_code = '01951' AND date = '2024-01-01';"
        },
        'data_dictionary': {
            'column_definitions': [],
            'relationships': [],
    ...
    if sample_df is not None:
        for col in sample_df.columns:
            ...
            # 确定字段类型和描述
            col_lower = col.lower()
            if 'main' in col_lower and 'flow' in col_lower:
                field_type = 'REAL'
                description = '主力资金流（万元）'
                constraint = ''
                example = '1000.50'
            elif 'retail' in col_lower and 'flow' in col_lower:
                field_type = 'REAL'
                description = '散户资金流（万元）'
                constraint = ''
                example = '500.25'
            elif 'total' in col_lower and 'flow' in col_lower:
                field_type = 'REAL'
                description = '总资金流（万元）'
                constraint = ''
                example = '1500.75'
            elif 'buy' in col_lower or 'buying' in col_lower:
                field_type = 'REAL'
                description = '买入金额（万元）'
                constraint = ''
                example = '2000.00'
            elif 'sell' in col_lower or 'selling' in col_lower:
                field_type = 'REAL'
                description = '卖出金额（万元）'
                constraint = ''
                example = '1800.00'
                ...
                field_type = 'REAL'
                description = '净流入/流出（万元）'
                constraint = ''
                example = '200.00'
            elif 'price' in col_lower and ('pct' in col_lower or 'chg' in col_lower):
                field_type = 'REAL'
                description = '价格变化百分比（%）'
                constraint = ''
                example = '2.50'
    ...
    if table_exists(conn, 'hk_hist_weekly_moneyflow'):
        try:
            ...
            # 总记录数
            cursor.execute("SELECT COUNT(*) FROM hk_hist_weekly_moneyflow")
    ...
    data_dict['data_dictionary']['business_rules'] = [
        ...
        {
            'rule': '价格变化百分比特殊处理',
            'description': 'price_chgpct 列从CSV读取后乘以100，以百分比形式存储'
        },
        {
            'rule': '列名规则',
            'description': '所有列名统一转换为小写，便于查询'
        },
        {
            'rule': '资金流精度',
        ...
        {
            'rule': '数据完整性',
            'description': '所有资金流数据应包含完整的主力、散户资金流信息'
        }
    ]
    ...
    data_dict['data_dictionary']['relationships'] = [
        {
            ...
            'field': 'date, stock_code',
            'description': '可通过日期和股票代码与周K线数据关联'
        },
        {
            'type': '外键关联',
            'table': 'hk_hist_daily_moneyflow',
            'field': 'stock_code',
            'description': '可通过股票代码与日资金流数据关联'
        }
    ]
    ...
    if existing_dict:
        ...
        if 'metadata' in existing_dict:
            existing_dict['metadata']['updated_date'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            existing_dict['metadata']['version'] = '1.1'  # 更新版本号
            existing_dict['metadata']['description'] = '港股周资金流历史数据表（所有列名小写）'
        
        # 更新现有字典
        for key in data_dict:
            if key in existing_dict:
                # 对于列表类型，合并
                if isinstance(data_dict[key], list) and isinstance(existing_dict[key], list):
                    existing_dict[key] = data_dict[key] + existing_dict[key]
                # 对于字典类型，递归合并
                elif isinstance(data_dict[key], dict) and isinstance(existing_dict[key], dict):
                    existing_dict[key] = data_dict[key]
                else:
                    existing_dict[key] = data_dict[key]
            else:
                existing_dict[key] = data_dict[key]
        
        data_dict = existing_dict
        print("[INFO] 已合并到现有数据字典")
...
def main():
    ...
    if not csv_files:
        print(f"[ERROR] 在 {DATA_DIR} 中未找到 *_weekly_moneyflow.csv 文件")
    ...
    try:
        # 检查表是否存在
        if table_exists(conn, 'hk_hist_weekly_moneyflow'):
            existing_count = conn.execute("SELECT COUNT(*) FROM hk_hist_weekly_moneyflow").fetchone()[0]
            if existing_count > 0:
                print(f"\n[WARN] 表 hk_hist_weekly_moneyflow 中已有 {existing_count} 条记录")
                print("选项:")
                print("  1. 删除表并重新创建 (y) - 将创建新表结构（所有列名小写）")
                print("  2. 清空现有数据并重新导入 (c)")
                print("  3. 保留现有数据，追加新数据 (n)")
                response = input("请选择 (y/c/n): ").strip().lower()
                ...
                if response == 'y' or response == 'yes':
                    ...
                    clear_table(conn, 'hk_hist_weekly_moneyflow')
                    print("[INFO] 已清空旧数据")
                else:
                ...
                # 获取表结构
                cursor = conn.cursor()
                cursor.execute("PRAGMA table_info(hk_hist_weekly_moneyflow)")
        ...
        # 显示表信息
        if table_exists(conn, 'hk_hist_weekly_moneyflow'):
            count = conn.execute("SELECT COUNT(*) FROM hk_hist_weekly_moneyflow").fetchone()[0]
            print(f"\n📋 表 hk_hist_weekly_moneyflow 当前共有 {count} 条记录")
            
            # 显示表结构
            cursor = conn.cursor()
            cursor.execute("PRAGMA table_info(hk_hist_weekly_moneyflow)")
            columns = cursor.fetchall()
            print(f"📋 表结构（字段顺序，所有列名小写）:")
            for i, col in enumerate(columns, 1):
                print(f"   {i}. {col[1]} ({col[2]}) {'NOT NULL' if col[3] else ''}")
            
            # 显示索引
            cursor.execute("SELECT name, sql FROM sqlite_master WHERE type='index' AND tbl_name='hk_hist_weekly_moneyflow'")
            ...
            # 显示股票统计
            stocks = conn.execute(
                "SELECT DISTINCT stock_code, stock_name FROM hk_hist_weekly_moneyflow ORDER BY stock_code"
            ).fetchall()
            print(f"\n📈 包含 {len(stocks)} 只股票的数据:")
            for code, name in stocks:
                cnt = conn.execute(
                    f"SELECT COUNT(*) FROM hk_hist_weekly_moneyflow WHERE stock_code='{code}'"
                ).fetchone()[0]
                # 检查是否在配置文件中
